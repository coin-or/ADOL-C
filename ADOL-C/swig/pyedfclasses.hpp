...
public:
  ...
  int fov_forward(int n, double *x, int p, double **Xp, int m, double *y,
                  double **Yp) {
    PyObject *args = NULL;
    int rc;
    npy_intp in[1] = {n}, out[1] = {m};
    ...
    PyObject *ya = PyArray_SimpleNewFromData(1, out, NPY_DOUBLE, (void *)y);
    PyObject *xpa =
        PyArray_SimpleNewFromData(2, inp, NPY_DOUBLE, (void *)&Xp[0][0]);
    PyObject *ypa =
        PyArray_SimpleNewFromData(2, outp, NPY_DOUBLE, (void *)&Yp[0][0]);
    PyObject *no = PyInt_FromLong((long int)n);
    PyObject *mo = PyInt_FromLong((long int)m);
  ...
  int fov_reverse(int m, int q, double **U, int n, double **Z, double *x,
                  double *y) {
    PyObject *args = NULL;
    int rc;
    npy_intp in[1] = {n}, out[1] = {m};
    ...
    PyObject *ya = PyArray_SimpleNewFromData(1, out, NPY_DOUBLE, (void *)y);
    PyObject *ua =
        PyArray_SimpleNewFromData(1, outq, NPY_DOUBLE, (void *)&U[0][0]);
    PyObject *za =
        PyArray_SimpleNewFromData(1, inq, NPY_DOUBLE, (void *)&Z[0][0]);
    PyObject *no = PyInt_FromLong((long int)n);
    PyObject *mo = PyInt_FromLong((long int)m);
...
public:
  ...
  int fos_forward(int iArrLen, int *iArr, int n, double *x, double *xp, int m,
                  double *y, double *yp) {
    PyObject *args = NULL;
    int rc;
    npy_intp in[1] = {n}, out[1] = {m}, arrsz[1] = {iArrLen};
  ...
  int fov_forward(int iArrLen, int *iArr, int n, double *x, int p, double **Xp,
                  int m, double *y, double **Yp) {
    PyObject *args = NULL;
    int rc;
    npy_intp in[1] = {n}, out[1] = {m}, arrsz[1] = {iArrLen};
    npy_intp inp[2] = {n, p}, outp[2] = {m, p};
  ...
  int fos_reverse(int iArrLen, int *iArr, int m, double *U, int n, double *Z,
                  double *x, double *y) {
    PyObject *args = NULL;
    int rc;
    npy_intp in[1] = {n}, out[1] = {m}, arrsz[1] = {iArrLen};
  ...
  int fov_reverse(int iArrLen, int *iArr, int m, int q, double **U, int n,
  ...
  int call(int iArrLen, PyObject *pyarr, int n, advector &x, int m,
           advector &y) {
    int rc;
    PyArrayObject *array2 = NULL;
    int is_new_object2 = 0;
    npy_intp size[1] = {iArrLen};
    int *iArr = NULL;
    array2 = obj_to_array_contiguous_allow_conversion(pyarr, NPY_INT,
                                                      &is_new_object2);
    if (!array2 || !require_dimensions(array2, 1) ||
        !require_size(array2, size, 1))
      SWIG_fail;
...
PyEDF_iArr::~PyEDF_iArr() { delete cobj; }

int PyEDF_iArr::call(int iArrLen, int *iArr, int n, adouble *xa, int m,
                     adouble *ya) {
  return cobj->call(iArrLen, iArr, n, xa, m, ya);
}
int PyEDF_iArr::call(int iArrLen, int *iArr, int n, advector &x, int m,
                     advector &y) {
  return cobj->call(iArrLen, iArr, n, x, m, y);
}
int PyEDF_iArr::call(int iArrLen, PyObject *pyarr, int n, advector &x, int m,
                     advector &y) {
  int rc;
  PyArrayObject *array2 = NULL;
  int is_new_object2 = 0;
  ...
  int *iArr = NULL;
  array2 =
      obj_to_array_contiguous_allow_conversion(pyarr, NPY_INT, &is_new_object2);
  if (!array2 || !require_dimensions(array2, 1) ||
      !require_size(array2, size, 1))
    SWIG_fail;
  iArr = (int *)array_data(array2);
