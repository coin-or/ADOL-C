...
\autoref{trace} covers aspects of the trace of recorded data that ADOL-C uses to
evaluate arbitrarily high order derivatives. The discussion includes storage 
requirements and the tailoring of certain trace characteristics to fit specific
...
{\sf trace\_off(file)}
\end{center}
are recorded on a sequential data set called {\em trace}. Pairs of
...
particular trace for subsequent function or derivative evaluations.
Unless several traces need to be kept, ${\sf tag} =0$ may be used throughout.
The optional integer arguments {\sf keep} and
{\sf file} will be discussed in \autoref{trace}. We will refer to the
...
on distinct traces by changing the value of {\sf tag}. 
...
section and records them in the header of the trace.
%
%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
{\sf \{}\hspace{0.1 in}\= \> // beginning of function body \\
\>{\sf  short int tag = 0;} \>   // trace array and/or trace file specifier\\
...
are recorded on the trace and can later be checked to recognize 
whether a step to another level was taken during a forward sweep 
at different arguments than at tracing time.
...
their internal calculations are also recorded on the trace.
...
%
%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
\subsection{Reusing the Trace for Arbitrary Input Values}
\label{reuse_trace}
%
In some situations it may be desirable to calculate the value and
derivatives of a function at arbitrary arguments by using a trace of
...
corresponding trace files, into a computing environment that
...
point at which the trace was generated, provided there are
...
structure (compared to the situation at the  tracing point)
may have changed, e.g. while at tracing arguments
...
from the evaluation point at which the trace was generated.
\vspace*{1ex}
\end{minipage} \\ \hline
 -2 &
\begin{minipage}{12.5cm}
...
\caption{Return values around the tracing point}
\label{fi:tap_point}
\end{figure}          
...
tracing process simply has to be repeated by executing the active section again.
The crux of the problem lies in the fact that the trace records only
...
quadratures are never recorded on the trace. 
Therefore, when there are user-defined quadratures the retracing is necessary at each
new point. If there are only branches conditioned on {\sf adouble}
comparisons one may hope that re-tracing becomes unnecessary when
...
strictly sequential trace access and
...
test condition {\sf b} is recorded on the trace as a {\em constant} with its
...
%                                                                  
%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
\section{Numbering the Traces and Controlling the Buffer}
\label{trace}
%
...
trace array or trace file, in general trace, which may subsequently be
...
re-traced at each new argument. Similarly, if conditions on
{\sf adouble} values lead to a different program branch being taken at 
a new argument the evaluation process also needs to be re-traced at the 
new point. Otherwise, direct evaluation from
the trace by the routine {\sf function} (\autoref{optdrivers}) is 
likely to be
faster. The use of quadratures and the results of all comparisons on
{\sf adouble}s are recorded on the trace so that {\sf function} and other
forward routines stop and  return appropriate flags if their use without
prior re-tracing is unsafe. To avoid any re-tracing certain types of
branches can be recorded on the trace through
the use of conditional assignments  
described before in \autoref{condassign}. 

Several traces may be generated and kept simultaneously.
A trace array is used as a triplet of buffers or a trace file is generated if
...
The filesystem folder, where the traces files may be written to disk, 
...
For simple usage, {\sf trace\_on} may be called with only the trace
...
{\sf  trace\_off} to 1, the user may force a numbered  trace
file to be written even if the trace array (buffer) does not overflow.
If the argument {\sf file} is omitted, it 
defaults to 0, so that the trace array is written onto a trace file only 
if the length of any of the buffers exceeds {\sf [OLVT]BUFSIZE} elements.

After the execution of an active section, if a trace file was generated, i.e.,
...
the trace on which their respective computational task is to be performed.
By calling {\sf trace\_on} with different trace {\sf tag}s, one can create
several traces for various function evaluations and subsequently perform
...
one can generate two different traces by calling {\sf trace\_on} with
...
%
%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
\subsection{Examining the Trace and Predicting Storage Requirements }
\label{examiningTape}
%
...
The first argument {\sf tag} specifies the particular trace of
...
{\sf counts[6-13]}: & other internal information about the trace.
\end{tabular}
\]
...
forward mode involves apart from the trace buffers an array of 
...
active section, the corresponding trace with the identifier {\sf tag}
...
indicate the validity of the trace as explained in \autoref{reuse_trace}.
...
the trace instead of executing the corresponding source code:
%
\begin{tabbing}
\hspace{0.5in}\={\sf short int tag;} \hspace{1.1in}\= \kill    % define tab position
\>{\sf int function(tag,m,n,x,y)}\\
\>{\sf short int tag;}         \> // trace identification \\
...
having the corresponding trace at hand: 
%
\begin{tabbing}
\hspace{0.5in}\={\sf short int tag;} \hspace{1.1in}\= \kill    % define tab position
...
given by the trace labeled with {\sf tag} is provided internally.
...
during tracing. If this assumption is falsely made all drivers
while internally calling the forward mode evaluation will return the value -1 or -2 
as already specified in \autoref{reuse_trace}.  
%
%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
\]
defined by an autonomous right-hand side $F$ recorded on the trace.
...
$n \geq 2$ independents has been executed and traced. We may 
...
{\sf tag} pointing to a trace for the evaluation of the inverse function 
...
Given the trace of an active section and the coefficients $x_j$, 
...
in the trace interpretation. In detail, ADOL-C provides 
\begin{tabbing}
\hspace{0.5in}\={\sf short int tag;} \hspace{1.1in}\= \kill    % define tab position
...
\label{ScaCas}
%     
Given any correct trace, one may call from within
...
name of the file on which the trace was written. If the trace file does
not exist, {\sf forward} assumes that the relevant 
trace is still in core and reads from the buffers.
...
the function {\sf reverse} with \mbox{{\sf d} = {\sf keep} $-$ 1} and the same trace
...
equal to zero if {\sf reverse} directly follows the tracing of an active
...
comparisons as described above in \autoref{reuse_trace}.
...
control, the consequences for the trace generation using the ``basic''
tracing approach as implemented in ADOL-C so far are shown in the left part of
\autoref{fig:bas_tap}. 
\begin{figure}[htbp]  
\begin{center}
\hspace*{0.5cm}\includegraphics[width=5.8cm]{tapebasic}\hfill
\includegraphics[trim = 0mm 3mm 0mm 0mm, clip,width=5.8cm]{tapeadv} \hspace*{0.5cm}\ 
\end{center}
\hspace*{0.8cm} Basic tracing process \hspace*{4.3cm} Advanced tracing process
\caption{Different tracing approaches}
\label{fig:bas_tap}
\end{figure} 
As can be seen, the iterative process is completely
unrolled due to the tracing process. That is, the trace contains an internal representation of each 
time step. Hence, the overall trace comprises a serious amount of redundant
...
a {\em nested tracing} mechanism has been incorporated into ADOL-C as illustrated on
the right-hand side of \autoref{fig:bas_tap}. This new
capability allows the encapsulation of the time-stepping procedure
such that only the last time step $x_{N} = F(x_{N-1},u_{N-1})$ is traced as one
...
to be stored for a possibly necessary retracing during the derivative calculation
as explained below. 

Instead of storing the complete trace, only a very limited number of intermediate
...
number of checkpoints and time steps $N$. It is important to note that the overall trace
size is drastically reduced due to the advanced tracing strategy.  For the
implementation of this nested tracing we introduced 
a so-called ``differentiating context'' that enables \mbox{ADOL-C} to
handle different internal function representations during the tracing 
procedure and the derivative calculation. This approach allows the generation of a new
trace inside the overall trace, where the coupling of the different traces is based on
...
\hspace*{2cm}\= {\sf cpc.setDoubleFct(time\_step\_function);}\\
             ...
             \> {\sf // subtrace number for checkpointing} \\
             \> {\sf cpc.setAlwaysRetaping(false);}\\
             \> {\sf // always retrace or not ?}
\end{tabbing}
...
\>{\sf short int sub\_tape\_num;}         \> // trace identification for sub\_trace \\
...
\end{tabbing}
%
Here {\tt sub\_tape\_num} is an ADOL-C identifier for the subtrace that
...
Two macros are available that only differ in the way the global trace information is handled.
...
\end{tabbing}
Inside the parallel region, separate traces may then be created.
...
derivatives can be computed using the traces created in the serial and
parallel parts of the function evaluation, where user interaction is
required for the correct derivative concatenation of the various traces.
...
computations on traces, where large applications require the traces to be
...
mode.
This variant coexists with the more universal
trace based mode in the package. Up to version 2.2.1 the traceless forward differentiation 
could only be used for the computation of first order derivatives.
The latest versions also allow the calculation of higher order derivatives in the traceless mode. 
The following subsections describe
the source code modifications required to use the traceless forward mode of
ADOL-C.
%
\subsection{The computation of first order derivatives}
\label{tracelessfirstorder}
%
\subsubsection*{Modifying the Source Code}
%
...
\caption{Example for the traceless forward mode}
\label{fig:traceless}
\end{figure}
%
Changes to the source code that are necessary for applying the
traceless forward ADOL-C for first order derivatives are described
in the following two subsections, where the vector mode version
can be seen as an extension of the scalar mode.
%
\subsubsection*{The scalar mode}
%
To use the traceless forward mode for the computation of first order derivatives, one has to include one
of the header files \verb#adolc.h# or \verb#adouble.h#
where the latter should be preferred since it does not include the
trace based functions defined in other header files. Hence, including
...
Since the two ADOL-C forward mode variants trace-based and traceless,
are prototyped in the same header file, the compiler needs to know if a
traceless version and which one (first order derivatives/higher order derivatives) is intended.
For first order derivatives this can be done by defining a
preprocessor macro named {\sf ADOLC\_TRACELESS}. Note that it is 
important to define this macro before the header file is included.
Otherwise, the trace-based version of ADOL-C will be used. 
...
of type {\sf adouble}. The whole traceless functionality provided by
...
resulting from the inlining the traceless version for first order derivatives has its own namespace
\verb#adtl# (\textbf{ad} for \textbf{t}angent \textbf{l}inear). As a result four possibilities 
of using the {\sf adouble} type are available for the traceless version: 
\begin{itemize}
...
Additionally, the traceless forward mode variant of ADOL-C for first order derivatives
...
\begin{itemize}
  \item Using the constructor
   ...
   \item Setting dot values directly
         \begin{center}
	   \begin{tabular}{l}
...
Similar to the traceless scalar forward mode, the traceless vector forward
mode for first order derivatives is used by defining {\sf ADOLC\_TRACELESS}. Furthermore, one has to define
...
resulting vector mode. Just as {\sf ADOLC\_TRACELESS} the new macro
...
\begin{center}
  ...
  \end{tabular}
\end{center}
Note that the memory requirements that can be roughly determined by\\
({\sf NUMBER\_DIRECTIONS}$+1$)*(number of {\sf adouble}s)
is not changed by these statements.
...
\begin{center}
  ...
  \end{tabular}
\end{center}
Additionally, the traceless vector forward mode of ADOL-C for first order derivatives offers two
...
\caption{Example for the traceless vector forward mode}
\label{fig:modcode2}
\end{figure}
%
\subsection{The computation of higher order derivatives}
%
In addition to the traceless forward mode for computing first order derivatives
the latest versions of ADOL-C also include a traceless variant for the calculation
of higher order derivatives. The next section specifies which derivatives can
be computed with ADOL-C in this mode.

As in \autoref{forw_rev_ad}, let $F:\mathbb{R}^n\rightarrow\mathbb{R}^m$,
where $y=F(x)$, be a $d$ times continously differentiable, analytic function.
Throughout this section the subscripts represent
the degree of Taylor coefficients. For a given polynomial
\begin{eqnarray*}
 x(t)=x_0+tx_1+t^2x_2+\dots+t^dx_d\in\mathbb{R}^n
\end{eqnarray*}
the Taylor coefficients $y_i$, $i\in\{1,\dots,d\}$, of the expansion
\begin{eqnarray*}
 y(t)\equiv y_0+ty_1+t^2y_2+\dots+t^dy_d=F(x(t))+\mathcal{O}(t^{d+1})\in\mathbb{R}^m
\end{eqnarray*}
can be computed with ADOL-C in the traceless mode.
The following section describes the source code modifications required to use the
traceless forward mode of ADOL-C for higher order derivatives.
%
\subsubsection*{Modifying the Source Code}
%
To use the traceless forward mode for higher order derivatives one of the header
files \verb#adolc.h# or \verb#adouble.h# has to be included where the latter
should be preferred as described in detail at the beginning of \autoref{tracelessfirstorder}.
Before using the traceless variant for higher order derivatives
the preprocessor macro named  {\sf ADOLC\_TRACELESS\_HIGHER\_ORDER}
has to be defined. Note that it is important to define this macro before the
header file is included. Otherwise the trace based version of ADOL-C will be used.

Similar to the trace based forward version of ADOL-C all derivative calculations
are done by calls to overloaded operators in the traceless mode for higher order
derivatives. Therefore all independent, intermediate and dependent variables
must be declared with type {\sf adouble}.

To avoid name conflicts the traceless version for higher order derivatives has its
own namespace \verb#adhotl# (\textbf{ad} for \textbf{h}igher \textbf{o}rder 
\textbf{t}angent \textbf{l}inear). As a result four possibilities
of using the {\sf adouble} type are available for this traceless version:
\begin{itemize}
 \item Defining a new type:
\begin{center}
  \begin{tabular}{l}
    {\sf typedef adhotl::adouble adouble;}\\
    ...\\
    {\sf adouble tmp;}
  \end{tabular}
\end{center}
...
\begin{center}
  \begin{tabular}{l}
   {\sf \#define adouble adhotl::adouble}\\
   ...\\
   {\sf adouble tmp;}
  \end{tabular}
\end{center}
...
\end{itemize}
The advantages and disadvantages of the four variants are described in \autoref{tracelessfirstorder}.

After defining the variables only two things are left to be done. The number
$d$ of Taylor coefficients to be calculated has to be set and the independent
variables must be initialized. The traceless version of ADOL-C for higher order
derivatives offers a function named \mbox{{\sf setDegree}} for setting the number
of derivatives. For example, the calculation of 15 Taylor coefficients can be
done by the call
\begin{center}
   \begin{tabular}{l}
    {\sf adouble::setDegree(15);}
   \end{tabular}
\end{center}
By default {\sf degree=10} Taylor coefficients $y_i$ are computed.
The function {\sf getDegree} allows the user to obtain the value of
{\sf degree} at any time during the evaluation process.

A variable of type {\sf adouble} in the traceless mode for higher order derivatives
consists of two components: the value ({\sf double val}) and the derivative ({\sf double* derivative}).
The first one corresponds to the value of the zeroth coefficient of $x(t)$
wheras the second one contains the derivative values, i.e., the remaining
Taylor coefficients of $x(t)$. If one is interested in
the computation of the first $d$ Taylor coefficients $y_i$, $i\in\{1,\dots,d\}$,
for a given polynomial $x(t)\in\mathbb{R}^n$ one has to initialize the 
independent variables as follows:
First of all $n$ {\sf adouble}s are needed
\begin{center}
   \begin{tabular}{l}
    {\sf adouble ad\_x[n];}
   \end{tabular}
\end{center}
The entries of $x_0$ should be assigned to the first components 
of these {\sf adouble}s
\begin{center}
  \begin{tabular}{l}
   {\sf double x0[n];}\\
   ...\\
   {\sf for(int s=0; s$<$n; s++)}\\
   {\sf\rule{0.5cm}{0pt} ad\_x[s].val=x0[s];}
  \end{tabular}
\end{center}
The remaining coefficients have to be stored in the second components
of the independent {\sf adouble}s. Assume that the Taylor coefficients
$x_i$, $i\in\{1,\dots,d\}$, have the following structure
\begin{eqnarray*}
 x_i=\left(\begin{array}{c} x_{i_1} \\ x_{i_2} \\ \vdots \\ x_{i_n} \end{array}\right)\ \ i=1,\dots,d \enspace .
\end{eqnarray*}
Then the derivative array of the {\sf adouble ad\_x[s]},
$s\in\{0,\dots,n-1\}$ should contain the values $x_{1_{s+1}},x_{2_{s+1}},\dots,x_{d_{s+1}}$.
Performing the function evaluation with the above described initialization
and storing the solution in the array {\sf adouble ad\_y[m]} yields
the desired result. If the Taylor coefficients of $y(t)$ are of the form
\begin{eqnarray*}
 y_i=\left(\begin{array}{c} y_{i_1} \\ y_{i_2} \\ \vdots \\ y_{i_m} \end{array}\right) \ \  i=1,\dots,d\enspace,       \end{eqnarray*}
then the derivative array of the {\sf adouble ad\_y[l]} contains the
values $y_{1_{l+1}},\dots,y_{d_{l+1}}$.

There are several different ways to initialize {\sf adouble}s like for
example assigning the variable a {\sf double} value. The derivative
array is then automatically set to zero. Of course it is also possible
to assign an {\sf adouble} another {\sf adouble}.

Another possibility is to use a constructor. One can assign the first
component of an {\sf adouble} a {\sf double} value.
\begin{center}
  \begin{tabular}{l}
   {\sf adouble x(1.0);}
  \end{tabular}
\end{center}
This creates an {\sf adouble} which holds the value one in terms of
function evaluation. The derivative components are automatically set to zero.
Remember, the following command only creates an {\sf adouble} that
...
\begin{center}
  ...
  \end{tabular}
\end{center}
Another approach is to set the value and the derivative components of an {\sf adouble}, using
\begin{center}
  \begin{tabular}{l}
   {\sf double *z;}\\
   {\sf z=new double[degree];}\\
   \\
   {\sf for(int i=0, i$<$degree; i++)}\\
   {\sf\rule{0.5cm}{0pt} z[i]=...}\\
   \\
   {\sf adouble x(1.0,z);}
  \end{tabular}
\end{center}
This would create an {\sf adouble} with the first component equal
to one and the derivative component equal to the array {\sf z}. Of course it is
as well possible to use a constructor with another {\sf adouble}.
\begin{center}
  \begin{tabular}{l}
   {\sf double *z;}\\
   {\sf z=...}\\
  ...
  \end{tabular}
\end{center}
After using this command all values of the {\sf adouble y} contain
the values of the {\sf adouble x}.

Another possibility is to set the values of an {\sf adouble} directly.
The traceless forward mode of ADOL-C for higher order derivatives
offers a function named {\sf setValue} for setting the first component
without changing the derivative array. The first component can be obtained
at any time during the evaluation process by calling the {\sf getValue}
method. Similar functions are also availabe for the second component of an
{\sf adouble}. It is possible to set only one value of the derivative
array or the whole array by using the methods {\sf setOneADValue} or
{\sf setADValue}. The derivative array of an {\sf adouble} can be assigned to
the first unit vector by the following:
\begin{center}
  \begin{tabular}{l}
   {\sf adouble x;}\\
   {\sf double *v;}\\
   \\
   {\sf v=new double[adouble::getDegree()];}\\
   {\sf v[0]=1.0;}\\
   {\sf for(int i=1; i$<$adouble::getDegree(); i++)}\\
   {\sf\rule{0.5cm}{0pt} v[i]=0.0;}\\
   \\
   {\sf x.setADValue(v);}
  \end{tabular}
\end{center}
To set only one component of the derivative array, the position and the
value must be supplied, for example:
\begin{center}
  \begin{tabular}{l}
   {\sf adouble x;}\\
   {\sf x=1.0;}\\
   {\sf //set the 5th derivative value to 1.0}\\
   {\sf x.setOneADValue(4,1.0);}\\
  \end{tabular}
\end{center}

One can also obtain a pointer to the derivative array of an {\sf adouble}
by calling \mbox{{\sf getADValue}}. It is also possible to get only
one value of the derivative array by using the method {\sf getOneADValue}
and supplying the desired position.

The following example shows all changes which are necessary to apply the traceless
forward version of ADOL-C for higher order derivatives. We consider the function
\begin{eqnarray*}
 F :\mathbb{R}^3&\rightarrow&\mathbb{R}\\
 F(z_1,z_2,z_3)&=&z_1^3+2z_2^3+3z_3^3+z_1z_2+2z_2z_3+3z_1z_3+5z_2z_3^2
\end{eqnarray*}
and calculate  $\frac{\partial^2 F}{\partial z_3^2}$ in $(1, 1, 1)^T$.
The source code for the function evaluation is given in \autoref{fig:hocode}
and the modified source code is illustrated in \autoref{fig:hocode1}. The initialization
of the Taylor coefficients $x_i$ results from the equations in \eqref{eq:y_0y_1}.
\begin{figure}[!h!t!b]
  \framebox[\textwidth]{\parbox{\textwidth}{
  \begin{tabbing}
   \hspace*{-1cm} \= \kill
   ...
   \> {\sf using namespace std;}\\
   \\
   \> {\sf double poly (double *z)}\{\\
   \> {\sf \rule{0.5cm}{0pt}double y;}\\
   \> {\sf \rule{0.5cm}{0pt}y=pow(z[0],3.0)+2.0*pow(z[1],3.0)+3.0*pow(z[2],3.0)+z[0]*z[1]}\\
   \> {\sf \rule{0.5cm}{0pt}+2.0*z[1]*z[2]+3.0*z[0]*z[2]+5.0*z[1]*z[2]*z[2];}\\
   \> {\sf \rule{0.5cm}{0pt}return y;}\\
   \> {\sf \}}\\
   \\
   \> {\sf int main ()\{}\\ 
   \> {\sf \rule{0.5cm}{0pt}int n=3;}\\
   \\
   \> {\sf \rule{0.5cm}{0pt}double* z=new double[n];}\\
   \> {\sf \rule{0.5cm}{0pt}double y;}\\
   \\
   \> {\sf \rule{0.5cm}{0pt}//initialization of independent variables:}\\
   \> {\sf \rule{0.5cm}{0pt}for(int i=0; i$<$n; i++)}\\
   \> {\sf \rule{1cm}{0pt}z[i]=1.0;}\\
   \\
   \> {\sf \rule{0.5cm}{0pt}//function evaluation:}\\
   \> {\sf \rule{0.5cm}{0pt}y=poly(z);}\\
   \\
   \> {\sf \rule{0.5cm}{0pt}cout $<<$ "Value: " $<<$ y $<<$ endl;}\\
   \\
   \> {\sf \rule{0.5cm}{0pt}delete [] z;}\\
   \> {\sf \}}
  \end{tabbing}
  }}
  \caption{Example for traceless forward mode for higher order derivatives}
  \label{fig:hocode}
\end{figure}
...
\begin{figure}[!h!t!b]
  ...
  \begin{tabbing}
   \hspace*{-1cm} \= \kill
   \> {\sf \#define ADOLC\_TRACELESS\_HIGHER\_ORDER}\\
   \> {\sf \#include $<$adolc/adouble.h$>$}\\
   \> {\sf typedef adhotl::adouble adouble;}\\
   \> {\sf \#include $<$iostream$>$}\\
   \> {\sf using namespace std;}\\
   \\
   \> {\sf adouble poly (adouble *z)\{}\\
   \> {\sf \rule{0.5cm}{0pt}adouble y;}\\
   \> {\sf \rule{0.5cm}{0pt}y=pow(z[0],3.0)+2.0*pow(z[1],3.0)+3.0*pow(z[2],3.0)+z[0]*z[1]}\\
   \> {\sf \rule{0.5cm}{0pt}+2.0*z[1]*z[2]+3.0*z[0]*z[2]+5.0*z[1]*z[2]*z[2];}\\
   \> {\sf \rule{0.5cm}{0pt}return y;}\\
   \> {\sf \}}\\
   \\
   \> {\sf int main()\{}\\
   \> {\sf \rule{0.5cm}{0pt}int d=2, n=3;}\\
   \\
   \> {\sf \rule{0.5cm}{0pt}//number of Taylor coefficients y\_i}\\
   \> {\sf \rule{0.5cm}{0pt}adouble::setDegree(d);}\\
   \\
   \> {\sf \rule{0.5cm}{0pt}adouble *z = new adouble[n];}\\
   \> {\sf \rule{0.5cm}{0pt}adouble y;}\\
   \\
   \> {\sf \rule{0.5cm}{0pt}//initialization of independent variables}\\
   \> {\sf \rule{0.5cm}{0pt}for(int i=0; i$<$n; i++)}\\
   \> {\sf \rule{1cm}{0pt}z[i] = 1.0;}\\
   \\
   \> {\sf \rule{0.5cm}{0pt}//set Taylor coefficients x\_i}\\
   \> {\sf \rule{0.5cm}{0pt}z[2].setOneADValue(0,1.0);}\\
   \\
   \> {\sf \rule{0.5cm}{0pt}//function evaluation}\\
   \> {\sf \rule{0.5cm}{0pt}y = poly(z);}\\
   \\
   \> {\sf \rule{0.5cm}{0pt}cout $<<$ "Value of the derivative:" $<<$ 2.0*y.getOneADValue(1) $<<$ endl;}\\
   \\
   \> {\sf \rule{0.5cm}{0pt}delete [] z;}\\
   \> {\sf \}}\\
  \end{tabbing}
...
source code and link the object files to get the executable. The procedure
is the same for the traceless mode for first order derivatives and the 
traceless mode for higher order derivatives.
...
Since the traceless forward version of ADOL-C is implemented in the
...
\begin{center}
  ...
  \end{tabular}
\end{center}
The mentioned source codes {\sf traceless\_scalar.c}, {\sf traceless\_vector.c}
and {\sf traceless\_higher\_order.c} illustrating the use of the traceless
scalar and vector mode for first order derivatives and the traceless mode for higher
order derivatives can be found in the directory {\sf examples}.
...
\begin{itemize}
  ...
  \item Drawbacks:
    ...
    \item Main memory limitations\\
      ...
      the reason among other things. The memory requirements $M$ for the
      computation of first order derivatives can be determined roughly 
      as followed: 
      \begin{itemize}
	...
	\item Vector mode: $M=$(number of {\sf adouble}s)*({\sf
      ...
      For higher order derivatives, one obtains $M=$(number of {\sf adouble}s)$*(d+1)$ 
      where $d$ denotes the degree of the highest derivative.
    \item Compile time\\
...
\begin{tabular*}{10.5cm}{cp{9.5cm}}
  \boldmath $\rightarrow$ \unboldmath
                & provides functions to start/stop the tracing of
                  ...
                  trace statistics (see \autoref{examiningTape}); \\
  $\bullet$     & is included by the header \verb=<adolc/adouble.h>=.
\end{tabular*}
\\ \hline
\end{tabular}
...
generated trace to evaluate derivatives can either be programmed in ANSI-C
...
\begin{verbatim}
#include <adolc/adouble.h>               // use of active doubles and tracing
#include <adolc/drivers/drivers.h>       // use of "Easy to Use" drivers 
                                   // gradient(.) and hessian(.)
#include <adolc/taping.h>                // use of tracing
...
void main() {
...
trace_on(1);                       // tag =1, keep=0 by default
  ...
  delete[] x;                      
trace_off();
tapestats(1,tape_stats);           // reading of trace statistics
cout<<"maxlive "<<tape_stats[2]<<"\n";
...                                // ..... print other trace stats
...
%\label{gaussexam}
%
%The following example uses conditional assignments to show the usage of a once produced trace 
...
%as independents. The resulting trace can be
