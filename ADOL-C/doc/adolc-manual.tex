...
caller is responsible for deallocating it using {\sf delete[]} once the
...
\>{\sf int jac\_pat(tapeId,depen,indep,basepoint,crs)}\\
\>{\sf short int tapeId;}        \> // tape identification \\
\>{\sf int depen;}               \> // number of dependent variables $m$ \\
\>{\sf int indep;}               \> // number of independent variables $n$ \\
\>{\sf const double *basepoint;} \> // independent vector $x$ \\
\>{\sf std::span$<$uint *$>$ csr[m];}    \> // compressed row sparsity structure \\
\end{tabbing}
%
...
argument {\sf crs} must be an array of length {\sf depen} whose entries
...
column indices of the independent variables influencing the $i$th
dependent.
%
...
is returned.
%
Ownership of the row buffers allocated for {\sf crs} is transferred to
the caller or the tape infrastructure.  The user is responsible for
ensuring that this memory is released appropriately once it is no
longer required.
%
The driver routine is prototyped in the header file
\verb=<adolc/sparse/sparsedrivers.h>= and is included automatically by
