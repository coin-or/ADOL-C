...
\usepackage[maxbibnames=99,  % Never write 'et al.' in a bibliography
            ...
            \addbibresource{adolc-manual.bib}
            
\usepackage{tikz}
\usetikzlibrary{decorations.pathreplacing}   % For underbraces
...
header \verb=<adolc/adtb_types.h>=. The latter must be included for compilation
...
\verb=<adolc/adtb_types.h>= automatically.
...
{\sf adouble}s and with the header file \verb=<adolc/adtb_types.h>= included.  
...
For contiguous collections, ADOL-C also provides bulk overloads based
on \verb|std::span|:
\begin{center}
\verb|std::span<adouble>(x, n) <<= std::span<const double>(px, n);|\\
\verb|std::span<adouble>(y, m) >>= std::span<double>(py, m);|
\end{center}
These span overloads are equivalent to applying the scalar
\boldmath $\ll=$ \unboldmath and \boldmath $\gg=$ \unboldmath assignments element-wise,
require matching span sizes, and preserve the same ordering of
independent and dependent variables on the tape.
%
%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
header file \verb=<adolc/adtb_types.h>= included as described
...
The header file \verb=<adolc/adtb_types.h>=
...
\item 
Compile the codes after including the header file \verb=<adolc/adtb_types.h>=. 
\end{enumerate}
...
\end{center}
with {\sf counts} being an array of at least {\sf STAT\_SIZE} integers.
The first argument {\sf tag} specifies the particular tape of
interest. The components of {\sf counts} represent
\[
\begin{tabular}{ll}
{\sf counts[NUM\_INDEPENDENTS]}: & the number of independents, i.e.~calls to \boldmath $\ll=$ \unboldmath, \\
{\sf counts[NUM\_DEPENDENTS]}: & the number of dependents, i.e.~calls to \boldmath $\gg=$ \unboldmath, \\
{\sf counts[NUM\_MAX\_LIVES]}: & the maximal number of live active variables,\\
{\sf counts[TAY\_STACK\_SIZE]}: & the size of Taylor stack (number of overwrites),\\
{\sf counts[OP\_BUFFER\_SIZE]}: & the buffer size (a multiple of eight),
\end{tabular}
\]
\[
\begin{tabular}{ll}
{\sf counts[NUM\_OPERATIONS]}: & the total number of operations recorded,\\
{\sf counts[...]}: & other internal information about the tape.
\end{tabular}
\]
The values {\sf maxlive} = {\sf counts[NUM\_MAX\_LIVES]} and {\sf tssize} = {\sf counts[TAY\_STACK\_SIZE]}
...
The function
\begin{center}
{\sf printTapeStats(FILE* stream, short tag)}
\end{center}
can be used to write the information above in human-readable form to the specified stream.

The function
\begin{center}
{\sf tape\_doc(short tag, int n, int m, double* x, double* y)}
\end{center}
\[
\begin{tabular}{ll}
{\sf tag}: & tape number, \\
{\sf n}: & number of dependents \\
{\sf m}: & number if independents \\
{\sf x}: & {\sf n} values of dependents \\
{\sf y}: & {\sf m} values of indepentens
\end{tabular}
\]
can be used to write the recorded tape to a \LaTeX{} document named {\sf tape\_TAG.tex} with {\sf TAG} being the tape number.
This should only be used for small tapes to keep the size of the generated document within reasonable limits.
...
rely on the the coloring package ColPack (\verb=https://github.com/CSCsw/ColPack=) developed by the authors of \cite{GeMaPo05} and \cite{GeTaMaPo07}.
To use the sparse drivers presented below, ColPack must be installed either automatically (using {\sf -DENABLE\_SPARSE=1})
 or manually (providing {\sf -DColPack\_DIR}). The drivers are in the namespace {\sf ADOLC::Sparse}
%
\paragraph{Sparse Jacobians}
%
To compute the entries of a sparse Jacobian matrix in coordinate
format, ADOL-C provides the high-level driver
{\sf sparse\_jac} with an owning {\sf SparseMatrix} result.
%
In contrast to earlier driver interfaces, all algorithmic options are
selected at compile time via template parameters rather than by a
runtime {\sf options} array. This allows the compiler to optimize the
chosen strategy while keeping the user interface concise.
%
\begin{tabbing}
\hspace{0.5in}\={\sf short int tapeId;} \hspace{1.2in}\= \kill
\>{\sf int sparse\_jac(tapeId,depen,indep,repeat,basepoint,}\\
\>{\sf \hspace{1.25in} sparseJac)}\\
\>{\sf short int tapeId;}          \> // tape identification \\
\>{\sf int depen;}              \> // number of dependent variables (rows) $m$ \\
\>{\sf int indep;}              \> // number of independent variables (columns) $n$ \\
\>{\sf int repeat;}             \> // control of pattern and seed reuse\\
\>{\sf const double *basepoint;}\> // independent vector $x$ \\
\>{\sf ADOLC::Sparse::SparseMatrix \&sparseJac;} \> // coordinate-format sparse Jacobian \\
\end{tabbing}
%
The behavior of {\sf sparse\_jac} is fully determined by its template
parameters:
\begin{itemize}
  \item {\sf SparseMethod} selects the method used for sparsity pattern
        detection (default: index-domain propagation),
  \item {\sf CompressionMode} selects the compression strategy (default: column-wise compression),
  \item {\sf ControlFlowMode} determines how control flow in the
        computational graph is handled (default: safe mode),
  \item {\sf BitPatternPropagationDirection} selects the direction of
        bit-pattern propagation when applicable (default: automatic),
  \item {\sf MemoryHandler} selects whether the output container is resized
        automatically or whether caller-provided storage is overwritten in
        place (default: automatic).
\end{itemize}
%
For piecewise-linear abs-normal forms, ADOL-C also provides the overload
\begin{tabbing}
\hspace{0.5in}\={\sf short int tapeId;} \hspace{1.2in}\= \kill
\>{\sf int sparse\_jac$<$PiecewiseLinear\{\}$>$(tag,m,n,s,repeat,}\\
\>{\sf \hspace{1.55in} x,sparseANF)}\\
\end{tabbing}
This overload takes the additional argument {\sf s} for the number of
switching variables and computes the sparse extended Jacobian. The structured
result object {\sf ADOLC::Sparse::SparseANF} partitions that extended Jacobian
into sparse blocks {\sf Y}, {\sf J}, {\sf Z}, and {\sf L} and additionally
stores the recovered values {\sf y} and {\sf z} together with the dense
constants {\sf cy} and {\sf cz}. The rows are ordered as $[y;z]$, the columns
as $[x;|z|]$, and the numerical values correspond to the block matrix
\[
  \left[\begin{array}{cc}
    Y & J\\ Z & L
  \end{array}\right].
\]
Further details are given in \autoref{absdrivers}.
%
If {\sf repeat = 0}, the routine determines the sparsity pattern of the
Jacobian, generates a suitable seed matrix, stores the associated metadata
internally on the tape, and then performs numeric recovery.
%
For {\sf repeat $\neq$ 0}, the previously cached sparsity and seed
information is reused to compute the numerical values of
the Jacobian at the given point {\sf basepoint}.  Thus {\sf repeat} controls
only metadata rebuild versus reuse; numerical recovery happens in both cases.
%
The array {\sf basepoint} provides the current values of the independent
variables. It is required whenever tight control-flow analysis is
used; otherwise it may be {\sf nullptr}.
%
In {\sf MemoryHandler::Auto} mode the output container is resized
automatically to the recovered nonzero count. In {\sf MemoryHandler::Manual}
mode the caller must already provide correctly sized output storage. The
choices of {\sf repeat} and {\sf MemoryHandler} are orthogonal.
%
For the legacy C wrapper, the interface remains
\begin{tabbing}
\hspace{0.5in}\={\sf short int tapeId;} \hspace{1.2in}\= \kill
\>{\sf int sparse\_jac(tapeId,depen,indep,repeat,basepoint,}\\
\>{\sf \hspace{1.25in} nnz,rind,cind,values,options)}\\
\end{tabbing}
There, memory ownership for {\sf rind}, {\sf cind}, and {\sf values} follows
adapter semantics: the Jacobian is first recovered into {\sf SparseMatrix}
and then copied into the three output arrays. If the user supplies non-null
arrays and {\sf *nnz} already matches the recovered nonzero count, those arrays
are reused in place. Otherwise, fresh arrays are allocated with {\sf new[]}
and returned to the caller, who must release them with {\sf delete[]}. On
exit, {\sf *nnz} is always overwritten with the actual recovered nonzero count.
%
The routine returns a non-negative status value on success and a
negative error code on failure.
%
The driver routine is prototyped in the header file
\verb=<adolc/sparse/sparsedrivers.h>= and is included automatically by
the global header file \verb=<adolc/adolc.h>=.

         
\paragraph{Sparse Hessians}
%
To compute the entries of a sparse Hessian matrix in coordinate format,
ADOL-C provides the high-level driver {\sf sparse\_hess}.
%
As for the sparse Jacobian driver, all algorithmic options are selected
at compile time via template parameters rather than by a runtime
{\sf options} array.
%
\begin{tabbing}
\hspace{0.5in}\={\sf short int tapeId;} \hspace{1.2in}\= \kill
\>{\sf int sparse\_hess(tapeId,indep,repeat,basepoint,}\\
\>{\sf \hspace{1.35in} nnz,rind,cind,values)}\\
\>{\sf short int tapeId;}          \> // tape identification \\
\>{\sf int indep;}                 \> // number of independent variables $n$ \\
\>{\sf int repeat;}                \> // control of pattern and seed reuse \\
\>{\sf const double *basepoint;}   \> // independent vector $x$ \\
\>{\sf int *nnz;}                  \> // number of nonzero entries \\
\>{\sf unsigned int **rind;}       \> // row indices (coordinate format) \\
\>{\sf unsigned int **cind;}       \> // column indices (coordinate format) \\
\>{\sf double **values;}           \> // numerical nonzero values \\
\end{tabbing}
%
The behavior of {\sf sparse\_hess} is determined entirely by its template
parameters:
\begin{itemize}
  \item {\sf ControlFlowMode} selects the handling of control flow during
        sparsity pattern computation (safe or tight),
  \item {\sf RecoveryMethod} selects the ColPack recovery algorithm used
        for numerical Hessian reconstruction (indirect or direct).
\end{itemize}
%
The argument {\sf repeat} controls whether sparsity pattern detection
and seed generation are performed or reused.  If {\sf repeat = 0}, the
routine computes the Hessian sparsity pattern, generates and caches the
corresponding seed information on the tape, and sets {\sf nnz} to the
number of nonzero entries discovered.
%
For {\sf repeat $\neq$ 0}, the cached sparsity and seed information is
reused to compute the numerical values of the Hessian at the point
{\sf basepoint}.  In this case, {\sf nnz} must match the value obtained
during the initial call; otherwise, the routine reports an error.
%
The array {\sf basepoint} provides the current values of the independent
variables. It is required when tight control-flow analysis is selected;
otherwise it may be {\sf nullptr}.
%
The Hessian is returned in coordinate format via the arrays {\sf rind},
{\sf cind}, and {\sf values}, each of length {\sf nnz}.  These arrays
contain the row indices, column indices, and numerical values of the
nonzero entries, respectively.
%
Memory ownership for {\sf rind}, {\sf cind}, and {\sf values} follows the
semantics of the underlying recovery routines.  If the user supplies
non-null pointers, the corresponding user-managed memory is used.
Otherwise, unmanaged memory is allocated internally, and the caller is
responsible for releasing it using {\sf delete[]}.
%
The routine returns a non-negative status value on success and a
negative error code on failure.
%
...
\paragraph{Computation of Jacobian Sparsity Pattern}
%
ADOL-C provides a high-level driver for computing the sparsity pattern
of a Jacobian matrix in compressed row storage (crs) format:
{\sf jac\_pat}.  The routine determines which independent variables
influence each dependent variable and records this structure without
computing numerical derivative values.
%
All algorithmic choices are selected at compile time via template
parameters rather than through a runtime {\sf options} array.
%
\begin{tabbing}
\hspace{0.5in}\={\sf std::span$<$uint *$>$ \&compressedRowStorage;} \hspace{0.6in}\= \kill
\>{\sf int jac\_pat(tapeId,depen,indep,basepoint,compressedRowStorage)}\\
\>{\sf short int tapeId;}        \> // tape identification \\
\>{\sf int depen;}               \> // number of dependent variables $m$ \\
\>{\sf int indep;}               \> // number of independent variables $n$ \\
\>{\sf const double *basepoint;} \> // independent vector $x$ \\
\>{\sf std::span$<$uint *$>$ \&compressedRowStorage;} \> // span over CRS row pointers \\
\end{tabbing}
%
The behavior of {\sf jac\_pat} is determined by its template parameters:
\begin{itemize}
  \item {\sf SparseMethod} selects the method used to compute the
        sparsity pattern (index-domain or bit-pattern propagation)
  \item {\sf ControlFlowMode} determines how control flow in the
        computational graph is handled (safe mode, tight mode);
  \item {\sf BitPatternPropagationDirection} selects the direction of
        bit-pattern propagation when applicable (forward, reverse, or
        automatic selection).
\end{itemize}
%
For abs-normal forms, ADOL-C also provides the overload \\
{\sf jac\_pat$<$PiecewiseLinear\{\}$>$(tag,m,n,s,x,compressedRowStorage)},
which computes the sparsity pattern of the extended Jacobian. The rows are
ordered as $[y;z]$, and the column indices refer to the variables
$[x;|z|]$. Further details are given in \autoref{absdrivers}.
%
The array {\sf basepoint} provides the current values of the independent
variables and is required when tight control-flow analysis is selected;
otherwise it may be {\sf nullptr}.
%
The sparsity pattern is returned in compressed row storage format. The
argument {\sf compressedRowStorage} must be a span of length {\sf depen}
whose entries are pointers to arrays of type {\sf unsigned int}.  After
the call, {\sf compressedRowStorage[i][0]} contains the number of nonzero
entries in row $i$
($0 \le i < m$), and the subsequent entries
{\sf compressedRowStorage[i][1]}, \ldots,
{\sf compressedRowStorage[i][compressedRowStorage[i][0]]} store the
zero-based column indices of the independent variables influencing the
$i$th dependent.
%
The routine returns a non-negative value equal to the total number of
nonzero entries in the Jacobian sparsity pattern.  In case of failure, a
negative error code originating from the underlying propagation routine
is returned.
%
Ownership of the row buffers referenced by {\sf compressedRowStorage} is
transferred to the caller or the tape infrastructure.  The user is
responsible for ensuring that this memory is released appropriately once
...
\paragraph{Computation of Hessian Sparsity Pattern}
%
ADOL-C provides a driver routine for computing the sparsity pattern of a
Hessian matrix in compressed row storage (crs) format:
{\sf hess\_pat}. The routine determines which independent variables
interact in the second derivatives of a scalar-valued function.
%
All algorithmic choices are selected at compile time via template
parameters rather than by runtime control flags.
%
\begin{tabbing}
\hspace{0.5in}\={\sf short int tapeId;} \hspace{1.3in}\= \kill
\>{\sf int hess\_pat(tapeId,indep,basepoint,crs)}\\
\>{\sf short int tapeId;}        \> // tape identification \\
\>{\sf int indep;}               \> // number of independent variables $n$ \\
\>{\sf const double *basepoint;} \> // independent vector $x$ \\
\>{\sf std::span$<$uint *$>$ crs[n];}    \> // compressed row sparsity structure \\
\end{tabbing}
%
The behavior of {\sf hess\_pat} is determined by the template parameter
{\sf ControlFlowMode}, which selects how control flow in the
computational graph is handled during sparsity propagation
(safe, tight, oldsafe, oldtight).
%
The array {\sf basepoint} provides the evaluation point for tight
control-flow analysis and is required whenever a tight mode is selected;
otherwise it may be {\sf nullptr}.
%
The sparsity pattern is returned in compressed row storage format.  The
argument {\sf crs} must be an array of length {\sf indep} whose entries
are pointers to arrays of type {\sf unsigned int}. After the call,
{\sf crs[i][0]} contains the number of nonzero entries in row $i$
($0 \le i < n$), and the subsequent entries
{\sf crs[i][1]}, \ldots, {\sf crs[i][crs[i][0]]} store the zero-based
column indices of independent variables that interact with the $i$th
variable in the Hessian.
%
Before performing the computation, the routine resets the contents of
{\sf crs}. Memory allocated for the individual row buffers is owned by
the caller or the tape infrastructure and must be released
appropriately once it is no longer needed.
%
The routine returns a non-negative status value on success. In case of
failure, a negative error code forwarded from the underlying sparsity
propagation routine is returned.
%
The driver routine is prototyped in the header file
\verb=<adolc/sparse/sparsedrivers.h>= and is included automatically by
the global header file \verb=<adolc/adolc.h>=.
...
used as input for the derivative calculation. ADOL-C provides drivers
to generate such seed matrices from sparsity patterns given in
compressed row storage (crs) form. These drivers are thin wrappers
around the ColPack library.
%
In contrast to earlier interfaces, the compression orientation is
selected at compile time via template parameters rather than by a
runtime {\sf options} array.
%
\paragraph{Seed matrix for Jacobians.}
To generate a seed matrix for sparse Jacobian recovery, ADOL-C provides
\begin{tabbing}
\hspace{0.5in}\={\sf int m;} \hspace{1.3in}\= \kill
\>{\sf void generate\_seed\_jac(m,n,JP,Seed,p)}\\
\>{\sf int m;}                 \> // number of dependent variables $m$ \\
\>{\sf int n;}                 \> // number of independent variables $n$ \\
\>{\sf std::span$<$uint *$>$ JP;}   \> // crs sparsity pattern of the Jacobian \\
\>{\sf double ***Seed;}        \> // seed matrix (allocated by ColPack) \\
\>{\sf int *p;}                \> // compressed dimension \\
\end{tabbing}
%
The behavior of {\sf generate\_seed\_jac} is determined by the template
parameter {\sf CompressionMode} selecting the orientation of the
compression (column or row).
%
The input variables are the number of dependent variables {\sf m}, the
number of independent variables {\sf n}, and the Jacobian sparsity
pattern {\sf JP} in compressed row storage form, as for example computed
by {\sf jac\_pat}.  Based on {\sf JP}, the routine invokes ColPack's
bipartite graph coloring to construct a seed matrix {\sf Seed} suitable
for recovering the full Jacobian from compressed directional evaluations.
%
The output variable {\sf p} denotes the compressed dimension implied by
the coloring. Its interpretation depends on the selected compression
orientation: for row compression, {\sf p} is the number of columns of
the seed matrix; for column compression, {\sf p} corresponds to the
compressed dimension in the sense of ColPack's recovery setup.
%
The seed matrix {\sf Seed} is allocated using ColPack's unmanaged API
and returned to the caller through {\sf Seed}.  The caller is
responsible for managing and releasing this memory in accordance with
ColPack's unmanaged allocation semantics.
%
\paragraph{Seed matrix for Hessians.}
To generate a seed matrix for sparse Hessian recovery, ADOL-C provides
the driver
\begin{tabbing}
\hspace{0.5in}\={\sf int n;} \hspace{1.3in}\= \kill
\>{\sf void generate\_seed\_hess(n,HP,Seed,p)}\\
\>{\sf int n;}                 \> // number of independent variables $n$ \\
\>{\sf std::span$<$uint *$>$HP[n];}   \> // crs sparsity pattern of the Hessian \\
\>{\sf double ***Seed;}        \> // seed matrix (allocated by ColPack) \\
\>{\sf int *p;}                \> // compressed dimension \\
\end{tabbing}
%
The behavior of {\sf generate\_seed\_hess} is determined by the template
parameter {\sf RecoveryMethod}, which selects the recovery strategy:
\begin{itemize}
  \item {\sf Indirect}: indirect Hessian recovery using an acyclic
        coloring strategy,
  \item {\sf Direct}: direct Hessian recovery using a star coloring
        strategy.
\end{itemize}
%
The input variable {\sf HP} provides the Hessian sparsity pattern in
compressed row storage form, for example as computed by {\sf hess\_pat}.
Based on this pattern and the selected recovery strategy, the routine
invokes the corresponding ColPack coloring algorithm to construct a
seed matrix suitable for Hessian recovery.
%
The output variable {\sf p} denotes the compressed dimension implied by
the coloring.  The seed matrix {\sf Seed} is allocated using ColPack's
unmanaged API.  Ownership of the returned memory follows ColPack's
unmanaged semantics and must be respected by the caller.
\\

%
The specific criteria used to construct seed matrices depend on whether
the target derivative matrix is a Jacobian (nonsymmetric) or a Hessian
(symmetric), and on whether the recovery from the compressed
representation is performed directly or indirectly.  Appropriate
recovery routines are provided by ColPack and are used internally by
the drivers {\sf sparse\_jac} and {\sf sparse\_hess}.
%
These driver routines are prototyped in
\verb=<adolc/sparse/sparsedrivers.h>=, which is included automatically
by the global header file \verb=<adolc/adolc.h>= (see
\autoref{ssec:DesIH}).
%

\subsubsection{Legacy interface (runtime options).}
Earlier versions of ADOL-C provided non-templated interfaces with runtime
{\sf options} array. These interfaces are retained for backward
compatibility and select the underlying strategy at runtime.
New code should prefer the typed interface described above and we only refer to the 
documentation of release 2.7.2 for the legacy interface.
%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
prepares for {\sf fos\_reverse} or {\sf fov\_reverse} as explained below.
...
\text{\sf keep} = \left\{\begin{array}{cl}
       ...
       \end{array}\right.
\end{align*}
as explained below. For the {\bf f}irst-{\bf o}rder {\bf v}ector forward mode,
ADOL-C provides
\begin{tabbing}
\hspace{0.5in}\={\sf short int tag;} \hspace{1.1in}\= \kill    % define tab position
...
with an appropriate value of the parameter {\sf keep}, one may use the
...
$s=8*${\sf sizeof}$(${\sf bitword\_t}$)$ such Boolean vectors
...
$Z\in\N^{q \times n}$ corresponding to
\begin{equation}
\label{eq:int_forrev}
...
and $\tilde{Z}\in\{0,1\}^{\left(sq\right)\times n}$.
In general, the application of the bit pattern versions of
{\sf forward} or {\sf reverse} can be interpreted as
propagating dependencies between variables forward or backward, therefore
both the propagated integer matrices and the corresponding
Boolean matrices are called {\em dependence structures}.

\paragraph{Typed C++ interface.}
The recommended interface selects the control-flow handling at compile time
via the template parameter {\sf ControlFlowMode}:
\begin{itemize}
  \item {\sf Safe}: conservative dependence propagation (default),
  \item {\sf Tight}: dependence propagation sensitive to control flow, evaluated
        at a given basepoint.
\end{itemize}
The dependence structures are represented as arrays of {\sf bitword\_t}
and are passed as vectors of pointers, i.e.\ a packed matrix is provided
as a list of row pointers.

The forward-mode bit-pattern propagation routine is
\begin{tabbing}
\hspace{0.5in}\={\sf short int tapeId;} \hspace{1.1in}\= \kill
\>{\sf int forward(tapeId,m,n,p,x,X,y,Y)}\\
\>{\sf short int tapeId;}                 \> // tape identification \\
\>{\sf int m;}                            \> // number of dependent variables $m$\\
\>{\sf int n;}                            \> // number of independent variables $n$\\
\>{\sf int p;}                            \> // number of packed words propagated $p$\\
\>{\sf double x[n];}                      \> // basepoint values (tight mode only)\\
\>{\sf bitword\_t *X[n];}                 \> // input dependence structure (seed)\\
\>{\sf double y[m];}                      \> // function values (tight mode only)\\
\>{\sf bitword\_t *Y[m];}                 \> // output dependence structure $Y$ \\
\end{tabbing}
%
In safe mode, the basepoint {\sf x} and function values {\sf y} are not required.
In tight mode, {\sf x} must be provided; then the routine evaluates the
taped function at {\sf x} in order to resolve control-flow dependencies and
may write the corresponding function values into {\sf y}.

The corresponding reverse-mode bit-pattern propagation routine is
\begin{tabbing}
\hspace{0.5in}\={\sf short int tapeId;} \hspace{1.1in}\= \kill
\>{\sf int reverse(tapeId,m,n,q,U,Z)}\\
\>{\sf short int tapeId;}                 \> // tape identification \\
\>{\sf int m;}                            \> // number of dependent variables $m$\\
\>{\sf int n;}                            \> // number of independent variables $n$\\
\>{\sf int q;}                            \> // number of packed words propagated $q$\\
\>{\sf bitword\_t *U[q];}                 \> // input dependence structure $U$ \\
\>{\sf bitword\_t *Z[q];}                 \> // output dependence structure $Z$ \\
\end{tabbing}
%
yielding the dependence structure $Z$ for a given dependence structure $U$.
As in forward mode, safe propagation is conservative. Tight reverse propagation
is control-flow sensitive and relies on the taped execution state corresponding
to the selected basepoint; in practice this is typically established by a prior
evaluation in tight mode.

\paragraph{Legacy (mode-selected) interface.}
For backward compatibility, ADOL-C also provides the classic entry points
documented in earlier versions of this manual. These interfaces select the
control-flow handling at runtime using a {\sf mode} argument:
{\sf mode = 0} for safe (default) and {\sf mode = 1} for tight.
%
\begin{tabbing}
\hspace{0.5in}\={\sf short int tapeId;} \hspace{1.1in}\= \kill
\>{\sf int forward(tapeId,m,n,p,x,X,y,Y,mode)}\\
\>{\sf int reverse(tapeId,m,n,q,U,Z,mode)}\\
\end{tabbing}
%
These legacy routines do not allocate or free memory for the dependence
structures; all buffers are managed by the caller.
...
One can control the storage growth by the factor $p$ or $q$ using
``strip-mining'' for successive groups of columns or rows at a time, i.e.\
partitioning $X$ or $U$ appropriately as described for the computation of
Jacobians in \autoref{vecCas}. The strip-mining batch size can be influenced
by the constant {\sf PQ\_STRIPMINE\_MAX} in the sparse package configuration.
...
run time. However, if the parameters are not defined as \verb=adouble=
...
i.e., between the invocation of \verb=trace_on()= and \verb=trace_off()=.
...
\noindent
\hspace*{2cm}{\sf int euler\_step(short tapeId, int m, int n, double *x, double *y);} .
\medskip

\noindent
Note that the formal parameters in the signature have {\sf double} type, that is, 
...
\noindent
\hspace*{2cm}{\sf ext\_diff\_fct *edf = reg\_ext\_fct(outerTapeId, extTapeId, euler\_step);} .
\smallskip

\noindent
Here {\sf outerTapeId} is the tape on which the external call is recorded and
{\sf extTapeId} is the tape that represents the external function itself.
This returns a pointer to an {\sf ext\_diff\_fct} instance specific to the
registered function. Then, the user supplies the callback methods of type {\sf std::function} that are
needed by the ADOL-C drivers used later. The callback signatures follow the
corresponding public ADOL-C driver signatures. For example, to register the callbacks for zero-order
forward and first-order scalar reverse one may write
\begin{tabbing}
\hspace*{2cm}\= {\sf edf-$>$zos\_forward = zos\_for\_euler\_step;}\\
             \> {\sf edf-$>$fos\_reverse = fos\_rev\_euler\_step;} .
\end{tabbing}
\noindent
The corresponding callback implementations typically call the ADOL-C driver on
the external tape:
\begin{verbatim}
  auto zos_for_euler_step = [edf](short, int m, int n, int keep, double *yin, double *yout) {
    return zos_forward(edf->extTapeId, m, n, keep, yin, yout);
  };

  auto fos_rev_euler_step = [edf](short tapeId, int m, int n, double *u, double *z,
                           double *x, double *y) {
    zos_forward(edf->extTapeId, m, n, 1, x, y);
    findTape(edf->extTapeId).nestedReverseEval(true);
    const int rc = fos_reverse(edf->extTapeId, m, n, u, z);
    findTape(edf->extTapeId).nestedReverseEval(false);
    return rc;
  };
\end{verbatim}

\noindent
The reverse callbacks receive the primal values {\sf x} and {\sf y} explicitly.
No user-managed scratch buffers are required for {\sf ext\_diff\_fct} in this
interface.  The call to {\sf nestedReverseEval(true)} belongs on
{\sf edf-$>$extTapeId}, that is, on the inner tape that is actually
reverse-evaluated by the callback. This flag is only needed in first-order
reverse callbacks such as {\sf fos\_reverse} and {\sf fov\_reverse}. It tells
ADOL-C to accumulate the inner reverse contribution into the existing input
adjoints {\sf z} instead of overwriting them, which is required when the outer
reverse sweep has already contributed to the same adjoint entries before the
external callback is executed.

\medskip
To record an external-function operation on the tape, inside the {\sf trace\_on(outerTape)} - {\sf trace\_off} block, the call to the external function must be replaced by
\smallskip

\noindent
\hspace*{2cm}{\sf rc=call\_ext\_fct(edf, n, ax, m, ay);},
\medskip

\noindent
where {\sf ax} and {\sf ay} are {\sf adouble} arrays from which the data is passed to {\sf x} and {\sf y}.
The arrays passed to {\sf call\_ext\_fct} must each occupy contiguous active
locations in ascending order. If the arguments are assembled inside a larger
active section, it is often convenient to reserve the required block using
{\sf ensureContiguousLocations(n+m)} before allocating the corresponding
{\sf adouble} objects.

\medskip

\noindent
In practice the most common workflow is:
\begin{enumerate}
\item provide derivatives of the external function via ADOLC (taping on extTapeId) or define a closed form for the derivative;
\item implement driver callbacks that wire the derivative of the external function with the outer tape;
\item register {\sf ext\_diff\_fct} on the outer tape and assign the driver
callbacks that are needed later;
\item tape the outer computation, replacing the original call by
{\sf call\_ext\_fct};
\item call the derivative driver as usually.
\end{enumerate}

\noindent
The usage of the external function facility is illustrated by the
example \verb=ext_diff_func= contained in \verb=examples/additional_examples/ext_diff_func=. The integration tests in
\verb=boost-test/integration_tests/external_fct.cpp= provide additional
examples for three complementary scenarios: repeated Euler-step parity,
standard driver parity for representative problems, and a nested reverse
accumulation regression that demonstrates why {\sf nestedReverseEval} must be
set on the inner tape during reverse callbacks.
...
information as illustrated by the light gray rectangles in
\autoref{fig:bas_tap}.  
...
\noindent
\hspace*{2cm}{\sf int timeStepFunction(size\_t n, adouble *state);}\\
\hspace*{2cm}{\sf int timeStepFunction(size\_t n, double *state);}
\medskip
...
be {\em registered} by constructing a checkpointing context. Assuming the
outer tape identifier {\sf tapeId} and the checkpoint tape identifier
{\sf tapeIdCheck} are available, this is done with
\medskip

\noindent
\hspace*{2cm}{\sf ADOLC::CP::Context cpc(tapeId, tapeIdCheck, timeStepFunction);}.
\medskip

\noindent
This initializes the checkpointing context {\sf cpc}. Then,
the user has to provide the remaining checkpointing information
by the following commands:
\begin{tabbing}
\hspace*{2cm}\= {\sf cpc.setDoubleFct(timeStepFunction);}\\
             \> {\sf // passive variant of the time step function}\\
             \> {\sf cpc.setSaveFct(saveState);}\\
             \> {\sf // optional: save extra non-adouble checkpoint data}\\
             \> {\sf cpc.setRestoreFct(restoreState);}\\
             \> {\sf // optional: restore extra non-adouble checkpoint data}\\
             \> {\sf cpc.setNumberOfSteps(N);}\\
             \> {\sf // number of time steps to perform}\\
             \> {\sf cpc.setNumberOfCheckpoints(10);}\\
             \> {\sf // number of checkpoints} \\
...
\end{tabbing}
The save and restore hooks are optional. They are only needed if the time
step depends on additional non-{\sf adouble} state that must travel with a
checkpoint.
...
\noindent
\hspace*{2cm}{\sf int cpc.checkpointing(tapeId);}
\medskip
...
The presented interface is prototyped in the header file
...
%
One has to note that AD computes exact derivative information of the representation of
$F$ given by the computer program. Hence, in numerous cases AD provides exactly
what the user of AD requires, but one has to keep this difference in
mind. This is especially important if the computer program to evaluate $F(x)$
contains an iterative process, as for example a Newton iteration or a fixed
point iteration. In these cases, the black-box application of AD may not yield
the derivative information with the required accuracy. Furthermore, the
resulting memory requirement may be unacceptable for real-world problems as
for example large CFD codes to simulate a flow over an aircraft wing. For these
large scale applications, frequently one introduces a pseudo time-stepping to compute
a quasi-steady state using a given set of design parameters.
This simulation forms one main ingredient to optimize the airfoil for a
particular flight scenario. Hence, derivatives with respect
to the design parameters are required if a calculus-based
optimization method is applied. For this purpose, the pseudo time-stepping can be
interpreted as a fixed point iteration. Then, it is possible
to control the accuracy of the derivatives and to reduce the memory
requirement using either a two-phase approach or the
piggyback method as described for example in~\cite[Section~11.4]{GrWa08}.

The method of Reverse Accumulation proposed and analyzed in detail by
Christianson \cite{Chr94} is one example for a two-phase approach.
The Reverse Accumulation is available in ADOL-C. Run time measurements for a large
scale application, namely the derivative computation for the CFD code TAUij
provided by the DLR Braunschweig \cite{Hei06}, are
presented and analyzed in~\cite{SWGH08}.

\subsubsection{Iterative evaluation of derivatives of fixed point iterations}
...
for any given control $u\in\R^n$. Then the limit point $x_*\in\R^n$ fulfills the fixed
...
%
\subsubsection{Forward and reverse derivative iterations}
%
Applying the chain rule of differentiation to Equation~\eqref{eq:fixedpoint} yields
\begin{equation}
\label{eq:numderiv}
 \frac{d\phi}{du}(u) =
 \frac{dx_*}{du} = \frac{dF}{dx}(x_*,u)\frac{dx_*}{du} +
                       \frac{dF}{du}(x_*,u).
\end{equation}
This formula is a fixed point equation for the derivative
$\frac{d\phi}{du}(u) = \frac{dx_*}{du}$.
Analytically, the derivative $\frac{d\phi}{du}(u)$
can be expressed as
\begin{equation}
\label{eq:anderiv}
 \frac{d\phi}{du}(u) =
 \left(I - \frac{dF}{dx}(\phi(u),u)\right)^{-1}
 \frac{dF}{du}(\phi(u),u).
\end{equation}
However, a numerical computation according to this
formula is not appropriate. Using Automatic Differentiation (AD),
directional derivatives $\dot \phi = \frac{d\phi}{du}\dot u \in \R^{n}$
are evaluated for a given $\dot u \in \R^m$ with the forward mode. The reverse
mode yields adjoints $\bar u^T = \bar x^T \frac{d\phi}{du} \in \R^{m}$
for a given $\dot u \in \R^m$.
Denoting $\dot x_k = \frac{dx_k}{du}\dot u$ for $k=0,1,\ldots$ and applying
the forward mode differentiation,
we obtain a fixed-point iteration for Equation~\eqref{eq:numderiv}
\begin{equation}
\label{eq:xdot}
  \dot x_{k1} = \frac{dF}{dx}(x_*,u) \dot x_k +
                 \frac{dF}{du}(x_*,u)\dot u
   = F'(x_*,u)
    \begin{pmatrix} \dot x_k \\ \dot u \end{pmatrix}.
\end{equation}
If the derivative $\frac{dF}{dx}(x_*,u)$ describes a contraction, this iteration
converges to the unique fixed point $\dot x_*=\frac{d\phi(u)}{du}\dot u$.
As analyzed already in \cite{Chr94},  Equation~\eqref{eq:anderiv} yields,
for any given $\bar x\in\R^n$, the identity
$$
\bar x^T\frac{dx_*}{du} = \bar x^T
  \left(I - \frac{dF}{dx}(x_*,u)\right)^{-1}
  \frac{dF}{du}(x_*,u).
$$
Setting $\zeta^T := \bar x^T\left(I - \frac{dF}{dx}(x_*,u)\right)^{-1}$,
we get the fixed point equation
$$
  \zeta_*^T = \zeta_*^T\frac{dF(x_*,u)}{dx} + \bar x^T
$$
for the adjoint information. Hence, one obtains for $k=0,1,\ldots$ the iteration
\begin{align}
 \nonumber
 (\zeta_{k+1}^T, \bar u_{k+1}^T)
 & =
  \left(\zeta_k^T\frac{dF(x_*,u)}{dx} + \bar x^T, \;
        \zeta_k^T\frac{dF(x_*,u)}{du}\right) \\
 %
 \label{eq:ubar}
 & =
 \zeta_k^T F'(x_*,u) + (\bar x^T, 0^T).
\end{align}
which converges to the unique fixed point consisting of
$\zeta_*^T=\bar x^T\left(I - \frac{dF}{dx}(x_*,u)\right)^{-1}$
and
$\bar u_* = \zeta_*^T\frac{dF}{du}(x_*,u) = \bar x^T\frac{dx_*}{du} $.
As shown in \cite{Chr94}, the uniform rate of convergence of the
derivative fixed point iteration \eqref{eq:ubar} equals the
asymptotic rate of convergence of the original fixed point iteration
\eqref{eq:iteration}.

Using not a black-box approach but a slightly advanced technique, AD
provides for the fixed point iteration \eqref{eq:iteration} given
as a computer program the efficient evaluation of the terms $F'\dot x$
and $\zeta^TF'$ in equation \eqref{eq:xdot} and \eqref{eq:ubar}) respectively.
For this purpose, we apply a two-phase approach,
i.e., a splitting of the original iteration \eqref{eq:iteration}
and the derivative iteration \eqref{eq:xdot} and \eqref{eq:ubar}
respectively. Then the iterations computing the derivatives use information
of $F(x_*,u)$ at the previously computed fixed point $x_*$. This approach to
compute adjoint information was
proposed in \cite{Chr94} and applied for example in \cite{GKS03}.
As alternative, the derivative information can be computed in combination with
the original iteration to evaluate $x_*$. This is the so-called
piggy-back approach~\cite{GrWa08}.

\subsubsection{Embedded fixed point iteration}
\label{sec:emb}
%
Usually fixed point iterations are part of a larger computational
context. This fact can be described in a mathematical way as a composition of
mappings
\begin{equation}
\label{forwmapping}
 \underbrace{p \stackrel{\tau}{\mapsto} (x_0,u)
   \stackrel{\phi}{\mapsto} (x_*,u)
   \stackrel{\chi}{\mapsto}  y}_{y = J(p)},
\end{equation}
where to $p$ is a set of given parameters.
We refer $\tau$ as \emph{initialization} calculating the initial value
$x_0$ of the fixed point iteration and the design vector $u$ from the
parameter values $p$. The function $\phi$ denotes the \emph{fixed point iteration},
and $\chi$ represents the \emph{evaluation} of a target function to compute the target value
$y$. In the remainder of this subsection, we describe the application of the reverse
mode of AD to the computational procedure \eqref{forwmapping}.
The forward mode can be derived analogously.

The differentiation of $J(p) = (\chi\circ\phi\circ\tau)(p)$ using the reverse
mode of AD for a weight vector $\bar{y}$ yields the value $\bar p =
\bar{y}^T\bar{J}(p)$. Its computation can be seen as a composition
of the following functions:
\begin{align*}
     (\bar x_*, \bar u) &  = \bar{y}^T\bar \chi (x_*,u), \\
    \bar u             & = \bar u + \bar \phi (u,\bar x_*), \\
    \bar p             &  = \bar \tau (p,\bar u).
\end{align*}

Applying ADOL-C in a ``black-box'' fashion to the
computation of $J(p) = (\chi\circ\phi\circ\tau)(p)$ is not efficient since
it neglects any structure of the fixed point iteration in $\phi$.
According to the evaluation of $\bar \phi(u,\bar x_*) = \bar u$ in \eqref{eq:ubar},
we only require the repeated evaluation of the terms $\zeta_k^TF'(x_*,u)$.
Therefore the taping process is stopped during the evaluation of the fixed point
iteration $\phi()$. An internal function representation
of $F(x_*,u)$ is created only for the last iterate, which is assumed to
be sufficiently close to $x_*$. This internal representation of one iteration
of the fixed point process can be seen as a subtape.
An illustration of this advanced taping approach in comparison to the basic
taping, i.e., the black-box application of ADOL-C, is given in Figure~\ref{fig:bas_tap_iteration}.
\begin{figure}
 \begin{center}
  \begin{tikzpicture}[scale=0.5]
   % Basic taping
   \definecolor{gray102}{RGB}{102,102,102};
   \definecolor{gray153}{RGB}{153,153,153};
   \definecolor{gray204}{RGB}{204,204,204};

   \draw [fill,color=gray153] (0,0) rectangle ++(3,1.5);
   \draw [fill,color=gray204] (3,0) rectangle ++(2,1.5);
   \draw [fill,color=gray204] (7,0) rectangle ++(1,1.5);
   \draw [fill,color=gray102] (8,0) rectangle ++(3,1.5);

   \draw (0,0) rectangle ++(11,1.5);
   \foreach \x in {3,4,5,7,8}
     \draw (\x,0) -- ++(0,1.5);

   \foreach \x in {5.5,6,6.5}
     \node[circle,draw,color=black,fill=black,inner sep=0pt,minimum size=2] at (\x,0.75) {};

   \draw [-{Stealth[length=2.5mm]}] (0,2) to [edge label={tape generation}] (11,2);


   \draw [
    thick,
    decorate, decoration={brace, mirror, raise=0.1cm}
   ] (0,0) -- (3,0) node [pos=0.5,anchor=north,yshift=-0.25cm] {$\tau$};

   \draw [
    thick,
    decoration={brace, mirror, raise=0.1cm}, decorate
   ] (3,0) -- (8,0) node [pos=0.5,anchor=north,yshift=-0.2cm] {$\phi$};
   \draw [
    thick,
    decoration={brace, mirror, raise=0.1cm},
    decorate
   ] (8,0) -- (11,0) node [pos=0.5,anchor=north,yshift=-0.25cm] {$\chi$};

   \node at (5.5,-4) {Basic taping process};

   % Advanced taping
   \draw [fill,color=gray153] (13,0) rectangle ++(3,1.5);
   \draw [fill,color=gray102]    (21,0) rectangle ++(3,1.5);

   \draw (13,0) rectangle ++(11,1.5);
    \foreach \x in {3,4,5,7,8}
     \draw (13+\x,0) -- ++(0,1.5);

   \draw [fill=gray204] (20,-2.8) rectangle ++(1,1.5);
   \draw [-{Stealth[length=2.5mm]}] (20,0) -- ++(0,-1.3);
   \draw [-{Stealth[length=2.5mm]}] (21,-1.3) -- ++(0,1.3);

   \foreach \x in {5.5,6,6.5}
     \node[circle,draw,color=black,fill=black,inner sep=0pt,minimum size=2] at (13+\x,0.75) {};

   \draw [-{Stealth[length=2.5mm]}] (13,2) to [edge label={tape generation}] (24,2);

   \node at (18.5,-4) {Advanced taping process};
  \end{tikzpicture}

 \end{center}
\caption{Different taping approaches}
\label{fig:bas_tap_iteration}
\end{figure}
During the evaluation of the adjoint information $\bar p = \bar{y}^T\bar{J}(p)$
the tape is interpreted in the reverse order starting at the end.
Exploiting the advanced taping mechanism, the original derivative computation is interrupted
when the fixed point iteration is reached. The reverse mode differentiation
of the fixed point iteration according to (\ref{eq:ubar}) is performed
by repeatedly computing the reverse mode of $F(x_*,u)$ for different
weight vectors $\zeta_k$. After the computation of $\bar u$, the original
derivative computation is started again to evaluate the overall derivative
information $\bar p$.
This nested tape evaluation is illustrated in
Figure \ref{fig:deriv_calc}.
\begin{figure}
 \begin{center}
  \begin{tikzpicture}[scale=0.8]
   ...
   \draw [{Stealth[length=2.5mm]}-] (0,2) to [edge label={derivative computation}] (11,2);


   \draw [
    thick,
    decoration={brace, mirror, raise=0.1cm}, decorate
   ] (0,0) -- (3,0) node [pos=0.5,anchor=north,yshift=-0.25cm] {$\bar \tau$};

   \draw [
    thick,
    decoration={brace, mirror, raise=0.1cm}, decorate
   ] (4,-3.3) -- (7,-3.3) node [pos=0.5,anchor=north,yshift=-0.2cm] {$\bar \phi$};
   \draw [
    thick,
    decoration={brace, mirror, raise=0.1cm},
    decorate
   ] (8,0) -- (11,0) node [pos=0.5,anchor=north,yshift=-0.25cm] {$\bar \chi$};

   \draw [dashed] (7.5,-2.2) -- (8.2,-2.2);
   \draw [dotted] (7.5,-3.0) -- (8.2,-3.0);

   \node at (8.2,-2.2) [anchor=west] {switching the differentiation context};
   \node at (8.2,-3.0) [anchor=west] {reverse accumulation};

   % Advanced taping
   \draw  (4.5,-2.6) rectangle ++(2,2.1);
   \draw [fill=gray204] (5,-2.3) rectangle ++(1,1.5);
   \draw [-{Stealth[length=2.5mm]}, dashed] (4.5,-0.5) -- ++(-1.5,0.5);
   \draw [-{Stealth[length=2.5mm]}, dashed]      (8,0) -- ++(-1.5,-0.5);

   \draw [{Stealth[length=2.5mm]}-,dotted] (4.5,-0.5) arc[start angle=130, end angle=410,radius=1.55];
  \end{tikzpicture}
 \end{center}
\caption{Derivative computation exploiting the fixed point structure}
\label{fig:deriv_calc}
\end{figure}
%
...
\hspace{0.5in}\={\sf short int tag;} \hspace{1.1in}\= \kill    % define tab position
\>{\sf int
  fp\_iteration(}\={\sf sub\_tape\_num,double\_F,adouble\_F,norm,norm\_deriv,epsilon,epsilon\_deriv,}\\
\>              \>{\sf N\_max,N\_max\_deriv,x\_0,u,x\_fix,dim\_x,dim\_u)}\\
\hspace{0.5in}\={\sf short int tag;} \hspace{0.9in}\= \kill    % define tab position
\>{\sf int sub\_tape\_num;}         \> // tape identification for sub\_tape \\
\>{\sf int (*double\_F)(double*, double* ,double*, int, int);}     \\
\>                              \> // pointer to a function that computes for $x$ and $u$ \\
\>                              \> // the value $y=F(x,u)$ for {\sf double} arguments\\
\>{\sf int (*adouble\_F)(adouble*, adouble*, adouble*, int, int);} \\
\>                              \> // pointer to a function that computes for $x$ and $u$ \\
\>                              \> // the value $y=F(x,u)$ for {\sf adouble} arguments\\
\>{\sf double (*norm)(double*, int);}    \\
\>                              \> // pointer to a function that computes\\
\>{\sf double (*norm\_deriv)(double*, int);} \\
\>                              \> // pointer to a function that computes\\
\>{\sf int *norm\_deriv;}       \> // pointer to a function that computes\\
\>                              \> // the norm of a vector\\
\>{\sf double epsilon;}         \> // termination criterion for fixed point iteration\\
\>{\sf double epsilon\_deriv;}  \> // termination criterion for adjoint fixed point iteration\\
\>{\sf int N\_max;}             \> // maximal number of iterations for state computation\\
\>{\sf int N\_max\_deriv;}      \> // maximal number of iterations for adjoint computation\\
\>{\sf adouble *x\_0;}          \> // initial state of fixed point iteration\\
...
variables need to be initialized in the compiled part of the library
...
\begin{itemize}
  \item Advantages:
	...
	expected this way. However this is not guaranteed for repeated
        evaluations.
...
this can result in a long run time. To achieve a better performance one can
...
in parallel which can result in a faster wall-clock run time.
...
For the use of this mode a Cuda toolkit, which is suitable for the graphic card used,
...
Jacobian matrices. The independent variables have to be initialized, therefore 
...
\verb=-DENABLE\_SPARSE=1= command. The ColPack library is either automatically 
downloaded or can be provided specifying \verb=-DColPack\_DIR=.
...
\verb=-ladolc= in this case. An advanced user may in fact specify
...
Existing application determined dependencies between the provided 
...
%                   (see \autoref{arrays}); \\
%  $\bullet$     & is included by the header \verb=<adolc/adtb_types.h>=.
%\end{tabular*}
%\\ \hline
...
{\sf ADOLC\_DLL\_EXPORT adouble} in \verb=<adolc/adtb_types.h>=, add the statement
\begin{center}
...
\begin{verbatim}
#include <adolc/adtb_types.h>               // use of active doubles and taping
