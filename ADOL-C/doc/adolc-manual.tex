...
\begin{tabbing}
\hspace{0.5in}\={\sf std::span$<$uint *$>$ \&compressedRowStorage;} \hspace{0.6in}\= \kill
\>{\sf int hess\_pat(tapeId,indep,basepoint,compressedRowStorage)}\\
\>{\sf short int tapeId;}        \> // tape identification \\
\>{\sf int indep;}               \> // number of independent variables $n$ \\
\>{\sf const double *basepoint;} \> // independent vector $x$ \\
\>{\sf std::span$<$uint *$>$ \&compressedRowStorage;} \> // span over CRS row pointers \\
...
%
The sparsity pattern is returned in compressed row storage format.  The
argument {\sf compressedRowStorage} must be a span of length {\sf indep}
whose entries are pointers to arrays of type {\sf unsigned int}. After
the call, {\sf compressedRowStorage[i][0]} contains the number of
nonzero entries in row $i$
($0 \le i < n$), and the subsequent entries
{\sf compressedRowStorage[i][1]}, \ldots,
{\sf compressedRowStorage[i][compressedRowStorage[i][0]]} store the
zero-based column indices of independent variables that interact with the
$i$th variable in the Hessian.
%
Before performing the computation, the routine resets the contents of
{\sf compressedRowStorage}. Memory allocated for the individual row
buffers is owned by the caller or the tape infrastructure and must be
released appropriately once it is no longer needed.
%
...
\begin{tabbing}
\hspace{0.5in}\={\sf const std::span$<$uint *$>$ JP;} \hspace{0.8in}\= \kill
\>{\sf void generate\_seed\_jac(m,n,JP,Seed,p)}\\
\>{\sf int m;}                 \> // number of dependent variables $m$ \\
\>{\sf int n;}                 \> // number of independent variables $n$ \\
\>{\sf const std::span$<$uint *$>$ JP;} \> // CRS sparsity pattern of the Jacobian \\
...
the driver
\begin{tabbing}
\hspace{0.5in}\={\sf std::span$<$uint *$>$ HP;} \hspace{1.0in}\= \kill
\>{\sf void generate\_seed\_hess(n,HP,Seed,p)}\\
\>{\sf int n;}                 \> // number of independent variables $n$ \\
\>{\sf std::span$<$uint *$>$ HP;} \> // CRS sparsity pattern of the Hessian \\
\>{\sf double ***Seed;}        \> // seed matrix (allocated by ColPack) \\
\>{\sf int *p;}                \> // compressed dimension \\
\end{tabbing}
%
...
the external tape:
\begin{verbatim}
  auto zos_for_euler_step = [edf](short, int m, int n, double *yin, double *yout) {
    return zos_forward(edf->extTapeId, m, n, 0, yin, yout);
  };
