...
large scale applications, frequently one introduces a pseudo timestepping to compute
...
optimization method is applied. For this purpose, the pseudo timestepping can be
...
requirement using either a two-hase approach or the
piggyback method as described for example in~\cite[Section~11.4]{GrWa08}.

The method of Reverse Accumulation proposed and analysed in detail by
Christianson \cite{Chr94} as one example for a two-phase approach.
The Reverse Accumulation is available in ADOL-C. Runtime measurements for a large
...
presented and analysed in~\cite{SWGH08}.
...
\end{equation}
If the derivative $\frac{dF}{dx}(x_*,u)$ decribes a contraction, this iteration
converges to the unique fixpoint $\dot x_*=\frac{d\phi(u)}{du}\dot u$.
...
respectively. Then the iterations computing the derivaives use information
...
\label{sec:emb}
%
Ususally fixed point iterations are part of a larger computational
...
it neglegts any structure of the fixed point iteration in $\phi$.
