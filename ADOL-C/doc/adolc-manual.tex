...
tape based mode in the package. Up to version 2.2.1 the tapeless forward differentiation 
could only be used for the computation of first order derivatives.
The latest versions also allow the calculation of higher order derivatives in the tapeless mode. 
The following subsections describe
the source code modifications required to use the tapeless forward mode of
ADOL-C beginning with the mode for first order derivatives.
%
\subsection{The tapeless mode for first order derivatives}
%
...
tapeless forward ADOL-C for first order derivatives are described
in the following two subsections, where the vector mode version
is described as extension of the scalar mode.
%
\subsubsection*{The scalar mode}
%
To use the tapeless forward mode for first order derivatives, one has to include one
...
tapeless version and which one (first order derivatives/higher order derivatives) is intended.
...
resulting from the inlining the tapeless version for first order derivatives has its own namespace
...
Additionally, the tapeless forward mode variant of ADOL-C for first order derivatives
...
\begin{center}
  ...
  \end{tabular}
\end{center}
Additionally, the tapeless vector forward mode of ADOL-C for first order derivatives offers two
...
\end{figure}
%
\subsection{The tapeless mode for higher order derivatives}
%
In addition to the tapeless forward mode for computing first order derivatives
the latest versions of ADOL-C also include a tapeless variant for the calculation
...
\end{eqnarray*}
can be computed with ADOL-C in the tapeless mode.
The following sections describe the source code modifications required to use the
tapeless forward mode of ADOL-C for higher order derivatives.
%
\subsubsection{Modifying the Source Code}
%
To use the tapeless forward mode for higher order derivatives one of the header
files \verb#adolc.h# or \verb#adouble.h# has to be included where the latter
should be preferred as it does not contain the tape based functions defined in
other header files and hence avoids mode mixtures. The header file \verb#adolc.h#
is a wrapper for including all public headers of the ADOL-C package and does not
offer own functions. Consequently all tape based functions will be available after
including this header file which could lead to mode mixtures. Since the tape based
version is prototyped in the same header file the compiler still needs to know that
the tapeless version of ADOL-C will be used. Furthermore the user has to specify
which tapeless mode will be employed as the version for first order derivatives and the
one for higher order derivatives lead to different definitions of the class
{\sf adouble}. Before using the tapeless variant for higher order derivatives
the preprocessor macro named  {\sf ADOLC\_TAPELESS\_HIGHER\_ORDER}
has to be defined. Note that it is important to define this macro before the
header file is included. Otherwise the tape based version of ADOL-C will be used.

Similar to the tape based forward version of ADOL-C all derivative calculations
are done by calls to overloaded operators in the tapeless mode for higher order
derivatives. Therefore all independent, intermediate and dependent variables
must be declared with type {\sf adouble}.

To avoid name conflicts the tapeless version for higher order derivatives has its
own namespace \verb#adhotl#. As a result four possibilities
of using the {\sf adouble} type are available for this tapeless version:
\begin{itemize}
 \item Defining a new type:
\begin{center}
  \begin{tabular}{l}
  ...
  \end{tabular}
\end{center}
This is the preferred way. Remember, you cannot write an own {\sf adoube} type/class
with different meaning after doing the typedef.\\
\item Declaring with namespace prefix:
\begin{center}
  \begin{tabular}{l}
  ...
  \end{tabular}
\end{center}
Using this declaration is not very efficient with respect to coding but it is one of
the safest possibilities. The identifier {\sf adouble} is still available for user types/classes.\\
\item Using macros:
\begin{center}
  \begin{tabular}{l}
  ...
  \end{tabular}
\end{center}
This approach should be used carefully since standard defines are text replacements.\\
\item Using the complete namespace:
\begin{center}
   \begin{tabular}{l}
   ...
   \end{tabular}
\end{center}
A very clear approach with the disadvantage of uncovering all hidden secrets.
Name conflicts may arise!
\end{itemize}
After defining the variables only two things are left to be done. The amount
$d$ of Taylor coefficients to be calculated has to be set and the independent
variables must be initialized. The tapeless version of ADOL-C for higher order
derivatives offers a function named {\sf setDegree} for setting the number
of derivatives. The calculation of 15 Taylor coefficients for example can be
...
The next section describes the structure of a variable of type
{\sf adouble} in the tapeless mode for higher order derivatives
to facilitate the understanding of how to initialize independent
variables in a program.

A variable of type {\sf adouble} consists of two components: the value
({\sf double val}) and the derivative ({\sf double* derivative}).
The first one corresponds to the value of the zeroth coefficient of $x(t)$
wheras the second one contains the derivative values that is the remaining
Taylor coefficients of $x(t)$. Strictly speaking if one is interested in
...
independent variables in the program as follows:
First of all $n$ {\sf adouble}s are needed for example
\begin{center}
   \begin{tabular}{l}
    {\sf adouble ad\_x[n];}
   \end{tabular}
...
\begin{center}
  ...
  \end{tabular}
\end{center}
The remaining coefficients have to be saved in the second components
...
function evaluation. The second component is automatically set to zero.
...
\begin{center}
  ...
  \end{tabular}
\end{center}
Another approach is to set both components of an {\sf adouble}.
\begin{center}
  \begin{tabular}{l}
   {\sf double *z;}\\
...
to one and the second equal to the array {\sf z}. Of course it is
...
\begin{center}
  ...
  \end{tabular}
\end{center}
After using this command all values of the {\sf adouble x} are
assigned to the {\sf adouble y}.

Another possibility is to set the values of an {\sf adouble} directly.
The tapeless forward mode of ADOL-C for higher order derivatives
...
{\sf setADValue}. Assigning the derivative array of an {\sf adouble}
the first unit vector can be done as shown in the following example:
\begin{center}
  \begin{tabular}{l}
   {\sf adouble x;}\\
   {\sf double *v;}\\
  ...
  \end{tabular}
\end{center}
To set only one value of the derivative array, the position and the
...
by calling {\sf getADValue}. However, it is as well possible to get only
one value of the derivative array by using the method {\sf getOneADValue}
and supplying the desired position.

The following example shows all changes which are necessary to apply the tapeless
...
\begin{figure}[!h!t!b]
  ...
  \caption{Example for tapeless forward mode for higher order derivatives}
  \label{fig:hocode}
\end{figure}
...
\begin{figure}[!h!t!b]
  ...
  \begin{tabbing}
   \hspace*{-1cm} \= \kill
   \> {\sf \#define ADOLC\_TAPELESS\_HIGHER\_ORDER}\\
...
is the same for the tapeless mode for first order derivatives and the 
tapeless mode for higher order derivatives.
...
\begin{center}
  ...
  \end{tabular}
\end{center}
The mentioned source codes {\sf tapeless\_scalar.c}, {\sf tapeless\_vector.c}
and {\sf tapeless\_higher\_order.c} illustrating the use of the tapeless
scalar and vector mode for first order derivatives and the tapeless mode for higher
