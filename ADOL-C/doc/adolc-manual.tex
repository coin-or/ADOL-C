...
\>{\sf \hspace{1.25in} nnz,rind,cind,values)}\\
...
\>{\sf \hspace{1.55in} x,nnz,rind,cind,values)}\\
\end{tabbing}
This overload takes the additional argument {\sf s} for the number of
switching variables and computes the sparse extended Jacobian. The rows are
ordered as $[y;z]$, the columns as $[x;|z|]$, and the numerical values
correspond to the block matrix
\[
  \left[\begin{array}{cc}
    Y & J\\ Z & L
  \end{array}\right].
...
Jacobian, generates a suitable seed matrix, and stores the associated
metadata internally on the tape. The output variable {\sf nnz} is set
 by the routine to the number of nonzero entries discovered.
%
For {\sf repeat $\neq$ 0}, the previously cached sparsity and seed
information is reused to compute the numerical values of
the Jacobian at the given point {\sf basepoint}.  In this case, {\sf nnz}
must match the value obtained during the initial call; otherwise, the
routine reports an error.
%
...
%
The Jacobian is returned in coordinate format via the arrays
{\sf rind}, {\sf cind}, and {\sf values}, each of length {\sf nnz}.
The arrays contain the row indices, column indices, and numerical values
of the nonzero entries, respectively.
%
Memory ownership for {\sf rind}, {\sf cind}, and {\sf values} follows the
semantics of the underlying routines. If the user supplies
non-null pointers, the corresponding user-managed memory is used.
Otherwise, the routine allocates unmanaged memory internally, and the
caller is responsible for deallocating it using {\sf free()} once the
