...
...
using namespace std;
#define CMMCTR ADOLC_MPI_COMM_WORLD
#define tag 1

int main(int* argc,char** argv) {
    int n,i,j, id, size, root=0; // root process has id 0
    ...
    // every process knows the count independent variables

    int n_tmp = n/size;  // split count to #size arrays of length n_tmp
    double *xp = new double[n];
    double  yp = 0.0;
    adouble *x = new adouble[n], *x_tmp = new adouble[n_tmp];
    adouble  y , tmp=1;

    if( id == root)     // some initialization
       for(i=0; i<n; i++) xp[i] = (i+1.0)/(2.0+i); 

    trace_on(id, size, tag);  // tag = 1, keep = 0 by default
    if(id == root){
       for(i=0; i<n; i++)  x[i] <<= xp[i];  // or  x <<= xp outside the loop
       for(i=1; i< size; i++)
           ADOLC_MPI_Send(&x[i*n_tmp], n_tmp, MPI_ADOUBLE, i, tag, CMMCTR);
       for(i=0; i< n_tmp;i++) x_tmp[i] = x[i];
    } else
       ADOLC_MPI_Recv(x_tmp, n_tmp, MPI_ADOUBLE,root, tag, CMMCTR);
    /* now all independent variables are sent for computing */

    for(i=0; i < n_tmp ; i++) tmp *= x[i];

    ADOLC_MPI_Barrier(CMMCTR); // synchronise all processes
    ADOLC_MPI_Reduce( &tmp, &y , 1 , MPI_ADOUBLE, ADOLC_MPI_PROD, root, CMMCTR);
 
    if(id == root){
          y >>= yp;
          cout << The result is << yp << endl;
    }
    trace_off();   // end of tape

    double *grad = NULL;
    if ( id == root) grad = new double[n]; 
    gradient(id,size,tag, 1, n, x, grad);
    if(id == root){
        cout << "The gradient is "<< endl << "\t";
        for(i=0; i < n ; i++) cout << "  "<< grad[i];
        cout << endl;
        delete[] grad;
    }

    double **hess = NULL;
    if ( id == root) hess = myarrayI2(n); 

    hessian(id,size,tag, 1, n, x, hess);
    if(id == root){
        print_mat("Hessian",n,n,hess);
        myfreeI2(n,hess);
    }
    delete[] x;
    delete[] x_tmp;
