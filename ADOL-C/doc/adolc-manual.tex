...
%
%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
\subsection{Basic Calls of the Toolbox for Lie-Derivatives}
Nonlinear controller and observer design often require certain types of Lie derivatives.
These are total derivatives of tensor fields along a vector field.
The drivers for calculating Lie derivatives in ADOL-C presented in this section
were developed by the authors of \cite{Roeb05} and \cite{Roeb11}. 
They are prototyped in the header file {\sf $<$adolc\_lie.h$>$} and
allow the calculation of Lie derivatives of scalar, vector, covector fields and gradients of Lie 
derivatives of scalar or vector fields.
...
fields $\mathbf f : \Omega \rightarrow \mathbb{R}^n$, $\mathbf g: \Omega \rightarrow \mathbb{R}^n$
and $\mathbf h: \Omega\rightarrow \mathbb{R}^m$ (for $m=1$ this is the scalar field $h$) 
are defined on an open subset $\Omega\subseteq\mathbb{R}^n$. 

%There are three types of fields, which are going to be used. 
%The routine function allows to evaluate the desired function from the tape instead of
%executing the corresponding source code:
\subsubsection{Lie derivatives of a scalar field}
Consider the initial value problem
\begin{equation*}
\dot{\mathbf x}=\mathbf f(\mathbf x),\quad y=h(\mathbf x),\quad \mathbf x(0)=\mathbf x_0 \in \Omega\\
\end{equation*}
with the vector field $\mathbf f: \Omega \rightarrow \mathbb{R}^n$. To compute Lie derivatives
of the scalar field $h: \Omega\rightarrow \mathbb{R}$ along the vector field $\mathbf f$ 
\begin{equation}\label{eq:lie_scalar}
(L_\mathbf f^0h(\mathbf x_0),\cdots,L_\mathbf f^dh(\mathbf x_0))
\end{equation}
we need the trace numbers of the active sections of $\mathbf f$ and $h$, 
the number of independent variables $n$, the initial coefficients $\mathbf x_0$
and the highest derivative degree $d$. The values of the Lie derivatives are 
then stored in the array {\sf result} of length $d+1$.
...
To compute Lie derivatives of a smooth map $\mathbf h: \Omega\rightarrow \mathbb{R}^m$ with 
$\mathbf h(\mathbf x)=(h_1(\mathbf x),\cdots,h_m(\mathbf x))$ along the same vector field
$\mathbf f$ 

\begin{equation}\label{eq:lie_vectorial}
L_\mathbf f^k\mathbf h(\mathbf x_0)=
(L_\mathbf f^kh_1(\mathbf x_0),\cdots,L_\mathbf f^kh_m(\mathbf x_0)), \quad \text{for}\quad k = 0, \cdots, d,
\end{equation}
...
\>{\sf short Tape\_F;}         \> // trace identification of vector field $\mathbf f$ \\
\>{\sf short Tape\_H;}         \> // trace identification of vector field $\mathbf h$\\
...
\subsubsection{Gradients of Lie derivatives of a scalar field}
To compute the gradients of Lie derivatives of a scalar field $h$
\begin{equation}\label{eq:gradient_scalar}
(dL_\mathbf f^0h(\mathbf x_0),\cdots, dL_\mathbf f^dh(\mathbf x_0)),
\end{equation}
...
\>{\sf double x0[n];}          \> // values of independent variables $x_0$\\
\>{\sf short d;}               \> // highest derivative degree $d$\\
\>{\sf double result[n][d+1];} \> // resulting gardients of Lie derivatives\\
\>{\sf} \> // of a scalar field\\
\end{tabbing}  

For calculating the jacobians of Lie derivatives of the field 
$\mathbf h: \Omega\rightarrow \mathbb{R}^m$ with $\mathbf h(\mathbf x)=(h_1(\mathbf x),\cdots,h_m(\mathbf x) )$
\begin{equation}\label{eq:jacobian_vectorial}
dL_\mathbf f^k\mathbf h(\mathbf x_0)=
(dL_\mathbf f^k h_1(\mathbf x_0),\cdots, dL_\mathbf f^k h_m(\mathbf x_0)),\quad \text{for}\quad k=0,\cdots, d,
\end{equation}
one can use
...
\subsubsection{Lie derivatives of a covector field}
In a program the user does not have to distinguish between a vector field
$\mathbf f: \Omega \rightarrow \mathbb{R}^n$ and its covector field 
$\mathbf \omega: \Omega \rightarrow (\mathbb{R}^n)^*$. For convenience we also use
the same character $\mathbf h$, but it is defined as a covector field $\mathbf h: \Omega \rightarrow (\mathbb{R}^n)^*$.
Using this notation Lie derivatives of the covector field $\mathbf h$ along the vector field $\mathbf f$
\begin{equation}\label{eq:lie_covector}
L_\mathbf f^k\mathbf h(\mathbf x_0)=
(L_\mathbf f^k h_1(\mathbf x_0),\cdots, L_\mathbf f^k h_n(\mathbf x_0)),\quad \text{for}\quad k=0,\cdots, d,
\end{equation}
can be computed by

\begin{tabbing}
\hspace{0.5in}\={\sf short int tag;} \hspace{1.1in}\= \kill    % define tab position
\>{\sf int lie\_covectorv(Tape\_F, Tape\_H, n, x0, d, result)}\\
\>{\sf short Tape\_F;}         \> // trace identification of vector field $\mathbf f$ \\
\>{\sf short Tape\_H;}         \> // trace identification of covector field $\mathbf h$\\
\>{\sf short n;}               \> // number of independent variables $n$\\
\>{\sf double x0[n];}          \> // values of independent variables $x_0$\\
\>{\sf short d;}               \> // highest derivative degree $d$\\
...
We can calculate iterated Lie derivatives (Lie brackets) of the vector field 
$\mathbf g: \Omega \rightarrow \mathbb{R}^n$ along the vector field 
$\mathbf f: \Omega \rightarrow \mathbb{R}^n$ 
\begin{equation}\label{eq:lie_vector}
L_\mathbf f^k\mathbf g(\mathbf x_0)=ad_\mathbf f^k \mathbf g(\mathbf x_0)=
(ad_\mathbf f^k g_1(\mathbf x_0),\cdots, ad_\mathbf f^k g_n(\mathbf x_0)),\quad \text{for}\quad k=0,\cdots, d,
\end{equation}
using

\begin{tabbing}
\hspace{0.5in}\={\sf short int tag;} \hspace{1.1in}\= \kill    % define tab position
\>{\sf int lie\_bracketv(Tape\_F, Tape\_G, n, x0, d, result)}\\
\>{\sf short Tape\_F;}         \> // trace identification of vector field $\mathbf f$ \\
...
\subsection{Drivers for Lie-Derivatives}
In this section, all versions of Lie derivatives routines, which utilize the
overloading capabilities of C++, are listed. They are also prototyped in the header
file {\sf $<$adolc\_lie.h$>$}.

\subsubsection{The Scalar Case}
This procedure computes the Lie derivatives of a scalar field $h$ as in \eqref{eq:lie_scalar}. 

\begin{tabbing}
\hspace{0.5in}\={\sf short int tag;} \hspace{1.1in}\= \kill    % define tab position
\>{\sf int lie\_scalar(Tape\_F, Tape\_H, n, x0, d, result)}\\
\>{\sf short Tape\_F;}         \> // trace identification of vector field $\mathbf f$ \\
\>{\sf short Tape\_H;}         \> // trace identification of scalar field $h$\\
\>{\sf short n;}               \> // number of independent variables $n$ and $m=1$\\
\>{\sf double x0[n];}          \> // values of independent variables $x_0$\\
...
This function computes the gradients of Lie derivatives of $h$ as in \eqref{eq:gradient_scalar}. 

\begin{tabbing}
\hspace{0.5in}\={\sf short int tag;} \hspace{1.1in}\= \kill    % define tab position
\>{\sf int lie\_gradient(Tape\_F, Tape\_H, n, x0, d, result)}\\
\>{\sf short Tape\_F;}         \> // trace identification of vector field $\mathbf f$ \\
\>{\sf short Tape\_H;}         \> // trace identification of vector field $\mathbf h$\\
\>{\sf short n;}               \> // number of independent variables $n$ and $m=1$\\
\>{\sf double x0[n];}          \> // values of independent variables $x_0$\\
...
\subsubsection{The Vector Case}
This procedure computes the Lie derivatives of $\mathbf h$ as in \eqref{eq:lie_vectorial}. It 
supports both calling conventions, i.e., for $m=1$ and $m>=1$.

\begin{tabbing}
\hspace{0.5in}\={\sf short int tag;} \hspace{1.1in}\= \kill    % define tab position
\>{\sf int Lie\_scalar(Tape\_F, Tape\_H, n, m, x0, d, result)}\\
\>{\sf short Tape\_F;}         \> // trace identification of vector field $\mathbf f$ \\
\>{\sf short Tape\_H;}         \> // trace identification of scalar field $h$\\
\>{\sf short n;}               \> // number of independent variables $n$\\
\>{\sf short m;}               \> // number of dependent variables $m$\\
\>{\sf double x0[n];}          \> // values of independent variables $x_0$\\
\>{\sf short d;}               \> // highest derivative degree $d$\\
\>{\sf double result[m][d+1];} \> // resulting gradients of Lie derivatives\\
\>{\sf } \> // of vectorial scalar fields\\
\end{tabbing} 

This function computes the jacobians of Lie derivatives of $\mathbf h$ as in \eqref{eq:jacobian_vectorial}.
It supports both cases \eqref{eq:gradient_scalar} and \eqref{eq:jacobian_vectorial} at once.

\begin{tabbing}
\hspace{0.5in}\={\sf short int tag;} \hspace{1.1in}\= \kill    % define tab position
\>{\sf int Lie\_gradient(Tape\_F, Tape\_H, n, m, x0, d, result)}\\
\>{\sf short Tape\_F;}         \> // trace identification of vector field $\mathbf f$ \\
\>{\sf short Tape\_H;}         \> // trace identification of vector field $\mathbf h$\\
\>{\sf short n;}               \> // number of independent variables $n$\\
\>{\sf short m;}               \> // number of dependent variables $m$\\
...
Although we do not have to differentiate between scalar and vector cases when calculating 
Lie derivates of a covector field and Lie brackets, we still list them here 
(see \eqref{eq:lie_covector} and \eqref{eq:lie_vector}).

\begin{tabbing}
\hspace{0.5in}\={\sf short int tag;} \hspace{1.1in}\= \kill    % define tab position
\>{\sf int Lie\_covector(Tape\_F, Tape\_H, n, x0, d, result)}\\
\>{\sf short Tape\_F;}         \> // trace identification of vector field $\mathbf f$ \\
\>{\sf short Tape\_H;}         \> // trace identification of covector field $\mathbf h$\\
\>{\sf short n;}               \> // number of independent variables $n$\\
\>{\sf double x0[n];}          \> // values of independent variables $x_0$\\
...
An example how to use these drivers for the calculation of Lie derivatives can be found 
in the file {\sf GantryCrane.cpp} in the directory {\sf examples}.
%
%
%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++  
