 ...
 */
inline constexpr std::size_t dynamicDim = std::numeric_limits<std::size_t>::max();

template<class V, std::size_t m, std::size_t dim=dynamicDim>
class ADValue;
...
template<class F>
struct IsADValue : public std::false_type
{};
 ...
 */
template<class E>
constexpr bool IsADValue_t = IsADValue<E>::value;
...
static constexpr std::size_t derivativeCount(std::size_t dim, std::size_t maxOrder)
{
  if (maxOrder==1)
    return dim;
  if (maxOrder==2)
...
class JetDerivativeData<T, maxOrder, dynamicDim>
{
  ...
  T* alloc()
  {
    return reinterpret_cast<T*>(pool_->malloc());
  }
...
public:
  ...
  JetDerivativeData(std::size_t dim)
    : JetDerivativeData(dim, threadLocalPool(derivativeCount(dim, maxOrder)))
  {}
...
public:
  ...
  JetDerivativeData(std::size_t dim)
    : dimension_(dim)
    , derivatives_(derivativeCount(dim, maxOrder), 0)
  {}
...
template<class T, std::size_t maxOrder, std::size_t dim>
class Jet
  : public JetDerivativeData<T, maxOrder, dim>
{
...
public:
  ...
  template<class I>
  const auto& operator()(I i, I j) const
  {
    return derivatives_[dimension()*(1+i) +j];
  }

  template<class I>
  auto& operator()(I i, I j)
  {
    return derivatives_[dimension()*(1+i) +j];
  }
 ...
 * \tparam n Dimension of the domain space (defaults to dynamicDim for a dynamically selected value)
 */
...
class ADValue
{
  ...
  template<class F>
  void forEachDerivativeIndex(const F& f) const
  {
    f();
...
public:
  ...
  template<class T, class I,
    std::enable_if_t<(sizeof(I), dim)==dynamicDim, int> = 0>
  ADValue(const T& value, I k, std::size_t d)
    : jet_(d)
  {
    partial() = value;
    if constexpr(maxOrder>=1)
...
//    forEachDerivativeIndex([&](auto... i) {
    forEachDerivativeIndex(y.dimension(), [&](auto... i) {
      partial(i...) += y.partial(i...);
    });
    return *this;
  }

  template <class Other>
  ADValue& operator+=(const Other& y)
  {
    partial() += y;
    return *this;
  }
...
//    forEachDerivativeIndex([&](auto... i) {
  ...
  ADValue& operator/=(const ADValue& y)
  {
    ...
    if constexpr(maxOrder>=2)
    {
      ...
      for(auto i : range(dimension()))
        for(auto j : range(dimension()))
          Z(i,j) = (X(i,j)*Y()*Y() - X()*Y(i,j)*Y()
                    + 2*X()*Y(i)*Y(j) - (X(i)*Y(j) + X(j)*Y(i))* Y())/tmp;
    }
    if constexpr(maxOrder>=1)
    {
      auto tmp = Y()*Y();
...
template<class X, class Y,
  std::enable_if_t<not IsADValue_t<Y>, int> = 0>
 ...
 */
template<class X, class Y,
  std::enable_if_t<not IsADValue_t<X>, int> = 0>
auto pow(const X& x, const Y& y)
{
  using std::pow;
  using std::log;
 ...
 */
template<class V,
  std::enable_if_t<IsADValue_t<V>, int> = 0>
auto pow(const V& x, const V& y)
{
  ...
  if constexpr(V::maxOrder>=1)
  {
    ...
    if constexpr(V::maxOrder>=2)
    {
      ...
      for(auto i : range(x.dimension()))
        for(auto j : range(x.dimension()))
          Z(i,j) = tmp1*X(i,j) + tmp2*Y(i,j) + tmp3*X(i)*X(j) + tmp4*Y(i)*Y(j) + tmp5*(X(i)*Y(j)+Y(i)*X(j));
    }
  }
  return z;
}
