...
int computeSparseJac(short tag, int depen, int indep, int numSwitches,
  ...
  std::vector<double *> seedResults(tape.sJInfos().seedRows_);
  std::vector<double *> seedSwitches(tape.sJInfos().seedRows_);

  for (int i = 0; i < tape.sJInfos().seedRows_; i++) {
    results[i] = tape.sJInfos().B_[i];
    resultsSwitch[i] = tape.sJInfos().B_[i] + indep;
  ...
  MINDEC(ret_val, fov_pl_reverse(tag, depen, indep, numSwitches,
                                 tape.sJInfos().seedRows_, seedResults.data(),
...
int buildHessPatternAndSeed(short tag, int indep, const double *basepoint,
  ...
  *nnz = tape.sHInfos().nnzIn_;
  ...
  Tensor<double> Xppp_cont{static_cast<size_t>(indep),
                           static_cast<size_t>(tape.sHInfos().p_), 1};
  tape.sHInfos().Xppp_ = Xppp_cont.data();
  ...
  tape.sHInfos().Zppp_ = Zppp_cont.data();
  Matrix<double> Upp_cont{1, 2};
  tape.sHInfos().Upp_ = Upp_cont.data();
