...
ADOLC_API inline void setNumDir(const size_t p) {
#ifdef USE_ADTL_REFCOUNTING
  ...
  adouble::advalpool =
      new boost::pool<boost::default_user_allocator_new_delete>(
          (adouble::numDir + 1) * sizeof(double));
#endif
}

ADOLC_API inline size_t getNumDir() { return adouble::numDir; }

ADOLC_API inline double makeNaN() {
  return ADOLC_MATH_NSP::numeric_limits<double>::quiet_NaN();
}

ADOLC_API inline double makeInf() {
...
ADOLC_API inline adouble log(const adouble &a) {
  ...
  FOR_I_EQ_1_LTEQ_NUMDIR
  if (a.PRIMAL_VALUE > 0)
    tmp.ADVAL_I = a.ADVAL_I / a.PRIMAL_VALUE;
...
ADOLC_API inline adouble sqrt(const adouble &a) {
  ...
  FOR_I_EQ_1_LTEQ_NUMDIR
  if (a.PRIMAL_VALUE > 0)
    tmp.ADVAL_I = a.ADVAL_I / (tmp.PRIMAL_VALUE * 2);
...
ADOLC_API inline adouble sin(const adouble &a) {
  adouble tmp;
  double tmp2;
...
ADOLC_API inline adouble atan(const adouble &a) {
  ...
  if (tmp2 != 0)
    FOR_I_EQ_1_LTEQ_NUMDIR
  tmp.ADVAL_I = a.ADVAL_I * tmp2;
  else FOR_I_EQ_1_LTEQ_NUMDIR tmp.ADVAL_I = 0.0;
  return tmp;
}
...
ADOLC_API inline adouble atan2(const adouble &a, const adouble &b) {
  ...
  if (tmp4 != 0)
    FOR_I_EQ_1_LTEQ_NUMDIR
  tmp.ADVAL_I =
      (a.ADVAL_I * b.PRIMAL_VALUE - a.PRIMAL_VALUE * b.ADVAL_I) / tmp3 * tmp4;
  else FOR_I_EQ_1_LTEQ_NUMDIR tmp.ADVAL_I = 0.0;
  return tmp;
}
...
ADOLC_API inline adouble pow(const adouble &a, const adouble &b) {
  ...
  double tmp2 =
      b.PRIMAL_VALUE * ADOLC_MATH_NSP::pow(a.PRIMAL_VALUE, b.PRIMAL_VALUE - 1);
...
// not
inline bool adouble::operator!() const { return PRIMAL_VALUE == 0.0; }
...
ADOLC_API inline bool operator!=(const double v, const adouble &a) {
...
/*******************  getter / setter  **************************************/
inline adouble::operator double const &() const { return PRIMAL_VALUE; }

inline adouble::operator double &&() { return (double &&)PRIMAL_VALUE; }

inline adouble::operator double() { return PRIMAL_VALUE; }

inline double adouble::getValue() const { return PRIMAL_VALUE; }
