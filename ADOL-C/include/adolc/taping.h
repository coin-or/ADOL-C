...
enum TapeRemovalType { ADOLC_REMOVE_FROM_CORE, ADOLC_REMOVE_COMPLETELY };
...
/*
 * free location block sorting/consolidation upon calls to
 * ensureContiguousLocations happens when  the ratio between allocated and used
 * locations exceeds gcTriggerRatio or the allocated locations exceed
 * gcTriggerMaxSize
 */
ADOLC_DLL_EXPORT void setStoreManagerType(unsigned char loctypes);

ADOLC_DLL_EXPORT void setStoreManagerControl(double gcTriggerRatio,
                                             size_t gcTriggerMaxSize);
...
ADOLC_DLL_EXPORT int trace_on(short tnum, int keepTaylors, uint obs, uint lbs,
                              uint vbs, uint tbs, int skipFileCleanup = 0);
