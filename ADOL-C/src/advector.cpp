...
adubref::operator adub() const {
  ...
  if (ADOLC_CURRENT_TAPE_INFOS
          .traceFlag) { // old: write_assign_a(locat,location);
    put_op(ref_copyout);
...
adub adubref::operator++(int) {
  ...
  if (ADOLC_CURRENT_TAPE_INFOS
          .traceFlag) { // old: write_incr_decr_a(incr_a,location);
    put_op(ref_incr_a);
...
adubref &adubref::operator=(const badouble &x) {
  ...
  {
    if (ADOLC_CURRENT_TAPE_INFOS
            .traceFlag) { // old:  write_assign_a(location,x.location);
      put_op(ref_assign_a);
...
void adubref::declareDependent() { adub(*this).declareDependent(); }

adubref &adubref::operator+=(double coval) {
  ADOLC_OPENMP_THREAD_NUMBER;
  ADOLC_OPENMP_GET_THREAD_NUMBER;
  if (ADOLC_CURRENT_TAPE_INFOS
          .traceFlag) { // old: write_d_same_arg(eq_plus_d,location,coval);
...
adubref &adubref::operator+=(const badouble &y) {
  ...
  locint y_loc = y.loc();
  if (ADOLC_CURRENT_TAPE_INFOS
          .traceFlag) { // old: write_a_same_arg(eq_plus_a,location,y.location);
...
adubref &adubref::operator-=(const badouble &y) {
  ...
  locint y_loc = y.loc();
  if (ADOLC_CURRENT_TAPE_INFOS
          .traceFlag) { // old: write_a_same_arg(eq_min_a,location,y.location);
    put_op(ref_eq_min_a);
    ADOLC_PUT_LOCINT(y_loc);    // = arg
    ADOLC_PUT_LOCINT(location); // = res
...
adubref &adubref::operator*=(const badouble &y) {
  ...
  locint y_loc = y.loc();
  if (ADOLC_CURRENT_TAPE_INFOS
          .traceFlag) { // old: write_a_same_arg(eq_mult_a,location,y.location);
...
void condassign(adubref &res, const badouble &cond, const badouble &arg1,
                const badouble &arg2) {
  ADOLC_OPENMP_THREAD_NUMBER;
  ADOLC_OPENMP_GET_THREAD_NUMBER;
  if (ADOLC_CURRENT_TAPE_INFOS
          .traceFlag) { // old:
                        // write_condassign(res.location,cond.location,arg1.location,
    //		     arg2.location);
  ...
  if (ADOLC_GLOBAL_TAPE_VARS.store[cond.loc()] > 0)
    ADOLC_GLOBAL_TAPE_VARS.store[res.refloc] =
...
void condassign(adubref &res, const badouble &cond, const badouble &arg) {
  ...
  if (ADOLC_CURRENT_TAPE_INFOS
          .traceFlag) { // old:
                        // write_condassign2(res.location,cond.location,arg.location);
...
void condeqassign(adubref &res, const badouble &cond, const badouble &arg1,
                  const badouble &arg2) {
  ADOLC_OPENMP_THREAD_NUMBER;
  ADOLC_OPENMP_GET_THREAD_NUMBER;
  if (ADOLC_CURRENT_TAPE_INFOS
...
adub advector::operator[](const badouble &index) const {
  ...
  ADOLC_GLOBAL_TAPE_VARS.store[locat] =
      ADOLC_GLOBAL_TAPE_VARS.store[data[idx].loc()];
  return locat;
}
...
void adolc_vec_copy(adouble *const dest, const adouble *const src, locint n) {
  ADOLC_OPENMP_THREAD_NUMBER;
  ADOLC_OPENMP_GET_THREAD_NUMBER;
  if (dest[n - 1].loc() - dest[0].loc() != (unsigned)n - 1 ||
      src[n - 1].loc() - src[0].loc() != (unsigned)n - 1)
...
adub adolc_vec_dot(const adouble *const x, const adouble *const y, locint n) {
  ADOLC_OPENMP_THREAD_NUMBER;
  ADOLC_OPENMP_GET_THREAD_NUMBER;
  if (x[n - 1].loc() - x[0].loc() != (unsigned)n - 1 ||
      y[n - 1].loc() - y[0].loc() != (unsigned)n - 1)
    fail(ADOLC_VEC_LOCATIONGAP);
  locint res = next_loc();
  if (ADOLC_CURRENT_TAPE_INFOS.traceFlag) {
    put_op(vec_dot);
...
void adolc_vec_axpy(adouble *const res, const badouble &a,
                    const adouble *const x, const adouble *const y, locint n) {
  ADOLC_OPENMP_THREAD_NUMBER;
  ADOLC_OPENMP_GET_THREAD_NUMBER;
  if (res[n - 1].loc() - res[0].loc() != (unsigned)n - 1 ||
      x[n - 1].loc() - x[0].loc() != (unsigned)n - 1 ||
      y[n - 1].loc() - y[0].loc() != (unsigned)n - 1)
    fail(ADOLC_VEC_LOCATIONGAP);
  locint a_loc = a.loc();
  if (ADOLC_CURRENT_TAPE_INFOS.traceFlag) {
