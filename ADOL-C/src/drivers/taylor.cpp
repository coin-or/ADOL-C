...
void coeff(int p, int d, struct item *coeff_list) {
  ...
  for (index_coeff_list = 0; 1;
       index_coeff_list++) { /* travers coeff_list, i.e. create all j with |j| =
                                d. */
    ptr = NULL;
    for (order_im = 1; order_im <= d;
         order_im++) { /* travers all orders from 1 to d for i */
      im[p - 1] = 0;
      im[0] = order_im;
      while (1) { /* create all i with order order_im. */
          ...
          while (1) { /* create all k where 0<k<=i */
            for (i = p - 1; i >= 0; i--)
              if (km[i] < im[i]) {
                ...
                order_km++;
                binomiZ *= im[i] - km[i] +
                           1; /* for (i over k)/(i over k-1) = (i-k+1)/k */
                binomiZ /= km[i];
                break;
              } else {
                /* binomiZ doesn't change, for (i over k) = 1 if k=0 and also if
                 * k=i */
                order_km -= km[i];
                km[i] = 0;
              };
        ...
        if (fabs(sum) > 0) { /* Store coefficient */
          ...
          for (u = 0; u < p; u++) /* It is sum(binomial(i+k,j+k),k=0..n-1) = */
            if (im[u] !=
                0) /* = ((j+n)*binomial(i+n,j+n)-j*binomial(i,j))/(1+i-j) */
            {
              i = u + j;
              n = im[u];
              address += ((j + n) * binomi(i + n, j + n) - binomi(i, j) * j) /
                         (1 + i - j);
              j += n;
            };
          ptr->a = address;
        ...
        if ((im[p - 1] == order_im) || (p == 1))
          break;
        for (i = p - 2; im[i] == 0; i--)
          ; /* find first nonvanishing entry on the right. */
        im[i]--;
        im[i + 1] = im[p - 1] + 1;
        if (i != p - 2)
          im[p - 1] = 0;
...
int LUFactorization(double **J, int n, int *RI, int *CI) {
  ...
  for (k = 0; k < n; k++) {
    ...
    if (ZERO > v) {
      fprintf(DIAG_OUT, "Error:LUFactorisation(..): no Pivot in step %d (%E)\n",
              k + 1, v);
      return -(k + 1);
    }
