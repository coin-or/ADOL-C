...
int AMPI_Init_NT(int *argc, char ***argv) {
  ...
  ourADTOOL_AMPI_FPCollection.popReduceCountAndType_fp =
      &ADTOOL_AMPI_popReduceCountAndType;
  ...
  ourADTOOL_AMPI_FPCollection.popGScommSizeForRootOrNull_fp =
      &ADTOOL_AMPI_popGScommSizeForRootOrNull;
  ...
  ourADTOOL_AMPI_FPCollection.push_CallCodeReserve_fp =
      &ADTOOL_AMPI_push_CallCodeReserve;
  ourADTOOL_AMPI_FPCollection.pop_CallCode_fp = &ADTOOL_AMPI_pop_CallCode;
  ourADTOOL_AMPI_FPCollection.push_AMPI_Request_fp =
      &ADTOOL_AMPI_push_AMPI_Request;
  ourADTOOL_AMPI_FPCollection.pop_AMPI_Request_fp =
      &ADTOOL_AMPI_pop_AMPI_Request;
  ...
  ourADTOOL_AMPI_FPCollection.mapBufForAdjoint_fp =
      &ADTOOL_AMPI_mapBufForAdjoint;
  ourADTOOL_AMPI_FPCollection.setBufForAdjoint_fp =
  ...
  ourADTOOL_AMPI_FPCollection.setAdjointCountAndTempBuf_fp =
      &ADTOOL_AMPI_setAdjointCountAndTempBuf;
  ourADTOOL_AMPI_FPCollection.allocateTempBuf_fp = &ADTOOL_AMPI_allocateTempBuf;
  ourADTOOL_AMPI_FPCollection.releaseAdjointTempBuf_fp =
      &ADTOOL_AMPI_releaseAdjointTempBuf;
  ourADTOOL_AMPI_FPCollection.incrementAdjoint_fp =
      &ADTOOL_AMPI_incrementAdjoint;
  ...
  ourADTOOL_AMPI_FPCollection.allocateTempActiveBuf_fp =
      &ADTOOL_AMPI_allocateTempActiveBuf;
  ourADTOOL_AMPI_FPCollection.releaseTempActiveBuf_fp =
      &ADTOOL_AMPI_releaseTempActiveBuf;
...
locint startLocAssertContiguous(adouble *adoubleBuffer, int count) {
  ...
  if (count > 0) {
    ...
    assert(start + count - 1 ==
           (adoubleBuffer + count - 1)
               ->loc()); // buf must have consecutive ascending locations
  }
  return start;
}

void ADTOOL_AMPI_pushBcastInfo(void *buf, int count, MPI_Datatype datatype,
  ...
  if (ADOLC_CURRENT_TAPE_INFOS.traceFlag) {
    ...
    if (isDerivedType(dt_idx)) {
      ...
      ;
      bitCountToLastActive =
          (count - 1) * dtdata->extents[dt_idx] +
...
void ADTOOL_AMPI_popBcastInfo(void **buf, int *count, MPI_Datatype *datatype,
                              int *root, MPI_Comm *comm, void **idx) {
...
void ADTOOL_AMPI_pushReduceInfo(
    void *sbuf, void *rbuf, void *resultData,
  ...
  if (ADOLC_CURRENT_TAPE_INFOS.traceFlag) {
    ...
    if (pushResultData)
      ADTOOL_AMPI_pushDoubleArray(resultData, count);
    TAPE_AMPI_push_int(pushResultData);
...
void ADTOOL_AMPI_popReduceInfo(void **sbuf, void **rbuf, void **prevData,
                               void **resultData, int *count, MPI_Op *op,
                               int *root, MPI_Comm *comm, void **idx) {
  int popResultData;
  TAPE_AMPI_pop_MPI_Comm(comm);
  TAPE_AMPI_pop_int(root);
  ...
  if (popResultData)
    ADTOOL_AMPI_popDoubleArray((double *)(*resultData), count);
...
void ADTOOL_AMPI_pushSRinfo(void *buf, int count, MPI_Datatype datatype,
                            int endPoint, int tag,
                            enum AMPI_PairedWith_E pairedWith, MPI_Comm comm) {
  if (ADOLC_CURRENT_TAPE_INFOS.traceFlag) {
    int i, dt_idx = derivedTypeIdx(datatype);
    int activeVarCount, bitCountToFirstActive, bitCountToLastActive;
    if (isDerivedType(dt_idx)) {
...
void ADTOOL_AMPI_popSRinfo(void **buf, int *count, MPI_Datatype *datatype,
                           int *endPoint, int *tag,
                           AMPI_PairedWith_E *pairedWith, MPI_Comm *comm,
                           void **idx) {
  TAPE_AMPI_pop_MPI_Comm(comm);
...
void ADTOOL_AMPI_pushGSinfo(int commSizeForRootOrNull, void *rbuf, int rcnt,
                            MPI_Datatype rtype, void *buf, int count,
...
void ADTOOL_AMPI_popGSinfo(int commSizeForRootOrNull, void **rbuf, int *rcnt,
                           MPI_Datatype *rtype, void **buf, int *count,
                           MPI_Datatype *type, int *root, MPI_Comm *comm) {
  int i;
  TAPE_AMPI_pop_MPI_Comm(comm);
  ...
  if (*count == 0) {
    ...
    *buf = (void *)(&(ADOLC_CURRENT_TAPE_INFOS.rp_A[bufLoc]));
  if (commSizeForRootOrNull > 0) {
...
void ADTOOL_AMPI_pushGSVinfo(int commSizeForRootOrNull, void *rbuf, int *rcnts,
                             int *displs, MPI_Datatype rtype, void *buf,
                             int count, MPI_Datatype type, int root,
                             MPI_Comm comm) {
  if (ADOLC_CURRENT_TAPE_INFOS.traceFlag) {
    int i;
    ...
    for (i = 0; i < commSizeForRootOrNull; ++i) {
      ...
      if (rcnts[i] > 0) {
        if (minDispls > displs[i])
          minDispls = displs[i];
        if (endOffsetMax < displs[i] + rcnts[i])
          endOffsetMax = displs[i] + rcnts[i];
      }
      if (endOffsetMax == 0)
        minDispls = 0;
    }
...
void ADTOOL_AMPI_popGSVinfo(int commSizeForRootOrNull, void **rbuf, int *rcnts,
                            int *displs, MPI_Datatype *rtype, void **buf,
                            int *count, MPI_Datatype *type, int *root,
                            MPI_Comm *comm) {
  int i;
...
void ADTOOL_AMPI_push_CallCodeReserve(enum AMPI_CallCode_E thisCall,
...
void ADTOOL_AMPI_pop_CallCode(enum AMPI_CallCode_E *thisCall) { assert(0); }

void ADTOOL_AMPI_push_AMPI_Request(struct AMPI_Request_S *ampiRequest) {
  ADTOOL_AMPI_pushSRinfo(ampiRequest->buf, ampiRequest->count,
                         ampiRequest->datatype, ampiRequest->endPoint,
                         ampiRequest->tag, ampiRequest->pairedWith,
                         ampiRequest->comm);
  if (ADOLC_CURRENT_TAPE_INFOS.traceFlag) {
...
void ADTOOL_AMPI_pop_AMPI_Request(struct AMPI_Request_S *ampiRequest) {
  ...
  ADTOOL_AMPI_popSRinfo(&(ampiRequest->adjointBuf), &(ampiRequest->count),
                        &(ampiRequest->datatype), &(ampiRequest->endPoint),
                        &(ampiRequest->tag), &(ampiRequest->pairedWith),
                        &(ampiRequest->comm), &(ampiRequest->idx));
}
...
void *ADTOOL_AMPI_rawDataV(void *activeData, int commSize, int *counts,
                           int *displs) {
  void *ret = NULL;
  int nonNullCount = 0;
  int minDispls = INT_MAX;
  for (int i = 0; i < commSize; ++i) {
    if (counts[i] > nonNullCount)
...
void *ADTOOL_AMPI_packDType(void *indata, void *outdata, int count, int idx) {
  if (!isDerivedType(idx))
    return indata; /* not derived type, or only passive elements */
  ...
  for (j = 0; j < count; j++) {
    ...
    for (i = 0; i < dtdata->counts[idx]; i++) {
      ...
      out_addr = (char *)outdata + out_offset +
                 (int)dtdata->arrays_of_p_displacements[idx][i];
      in_addr = (char *)indata + in_offset +
                (int)dtdata->arrays_of_displacements[idx][i];
      if (ADTOOL_AMPI_isActiveType(datatype) == AMPI_ACTIVE) {
        memcpy(out_addr,
               ADTOOL_AMPI_rawData((void *)in_addr,
                                   &dtdata->arrays_of_blocklengths[idx][i]),
               sizeof(revreal) * dtdata->arrays_of_blocklengths[idx][i]);
      } else if (isDerivedType(dt_idx)) {
        ADTOOL_AMPI_packDType(in_addr, out_addr,
                              dtdata->arrays_of_blocklengths[idx][i], dt_idx);
      } else {
        if (datatype == MPI_DOUBLE)
          ...
          assert(0);
        memcpy(out_addr, in_addr, s * dtdata->arrays_of_blocklengths[idx][i]);
      }
    }
  }
  return outdata;
  ...
  for (j = 0; j < count; j++) {
    ...
    for (i = 0; i < dtdata->counts[idx]; i++) {
      ...
      in_addr = (char *)indata + in_offset +
                (int)dtdata->arrays_of_p_displacements[idx][i];
      if (ADTOOL_AMPI_isActiveType(datatype) == AMPI_ACTIVE) {
        memcpy(ADTOOL_AMPI_rawData((void *)out_addr,
                                   &dtdata->arrays_of_blocklengths[idx][i]),
               in_addr,
               sizeof(revreal) * dtdata->arrays_of_blocklengths[idx][i]);
      } else if (isDerivedType(dt_idx)) {
        ADTOOL_AMPI_unpackDType(in_addr, out_addr,
                                dtdata->arrays_of_blocklengths[idx][i], dt_idx);
      } else {
        if (datatype == MPI_DOUBLE)
          s = (int)sizeof(double);
...
void *ADTOOL_AMPI_rawAdjointData(void *activeData) { return activeData; }
...
void ADTOOL_AMPI_getAdjointCount(int *count, MPI_Datatype datatype) {
  ...
  if (isDerivedType(dt_idx))
    *count *= getDTypeData()->num_actives[dt_idx];
}

void ADTOOL_AMPI_setAdjointCount(struct AMPI_Request_S *ampiRequest) {
  /* for now we keep the count as is but for example in vector mode one would
   * have to multiply by vector length */
...
void ADTOOL_AMPI_setAdjointCountAndTempBuf(struct AMPI_Request_S *ampiRequest) {
  ...
  ampiRequest->adjointTempBuf = ADTOOL_AMPI_allocateTempBuf(
      ampiRequest->adjointCount, ampiRequest->datatype, ampiRequest->comm);
}

void *ADTOOL_AMPI_allocateTempBuf(int adjointCount, MPI_Datatype dataType,
                                  MPI_Comm comm) {
  size_t s = 0;
  void *buf;
  int dt_idx = derivedTypeIdx(dataType);
...
void ADTOOL_AMPI_releaseAdjointTempBuf(void *tempBuf) { free(tempBuf); }

void *ADTOOL_AMPI_allocateTempActiveBuf(int count, MPI_Datatype datatype,
                                        MPI_Comm comm) {
  int dt_idx = derivedTypeIdx(datatype);
  if (isDerivedType(dt_idx)) {
    ...
    for (j = 0; j < count; j++) {
      ...
      for (i = 0; i < dtdata->counts[dt_idx]; i++) {
        ...
        if (blocktype == MPI_UB || blocktype == MPI_LB)
          assert(0);
        block_offset =
            struct_offset + dtdata->arrays_of_displacements[dt_idx][i];
        if (blocktype == AMPI_ADOUBLE) {
          new ((void *)((char *)buf + block_offset))
              adouble[dtdata->arrays_of_blocklengths[dt_idx][i]];
        }
      }
    }
    return buf;
...
void ADTOOL_AMPI_releaseTempActiveBuf(void *buf, int count,
                                      MPI_Datatype datatype) {
  int dt_idx = derivedTypeIdx(datatype);
  if (isDerivedType(dt_idx)) {
      ...
      for (i = 0; i < dtdata->counts[dt_idx]; i++) {
        ...
        if (blocktype == AMPI_ADOUBLE) {
          for (k = 0; k < dtdata->arrays_of_blocklengths[dt_idx][i]; k++) {
            ((adouble *)((char *)buf + block_offset + k * sizeof(adouble)))
                ->~adouble();
          }
        }
      }
...
void *ADTOOL_AMPI_copyActiveBuf(void *source, void *target, int count,
                                MPI_Datatype datatype, MPI_Comm comm) {
  int s, k, dt_idx = derivedTypeIdx(datatype);
  if (ADTOOL_AMPI_isActiveType(datatype) == AMPI_ACTIVE) {
    for (k = 0; k < count; k++)
      ((adouble *)target)[k] = ((adouble *)source)[k];
  } else if (isDerivedType(dt_idx)) {
    int i, j, extent, struct_offset, block_offset;
    MPI_Datatype blocktype;
    derivedTypeData *dtdata = getDTypeData();
    ...
    for (j = 0; j < count; j++) {
      ...
      for (i = 0; i < dtdata->counts[dt_idx]; i++) {
        ...
        if (blocktype == MPI_UB || blocktype == MPI_LB)
          assert(0);
        block_offset =
            struct_offset + (int)dtdata->arrays_of_displacements[dt_idx][i];
        if (ADTOOL_AMPI_isActiveType(blocktype) == AMPI_ACTIVE) {
          for (k = 0; k < dtdata->arrays_of_blocklengths[dt_idx][i]; k++) {
            ((adouble *)((char *)target + block_offset))[k] =
                ((adouble *)((char *)source + block_offset))[k];
          }
        } else {
          if (blocktype == MPI_DOUBLE)
            s = sizeof(double);
          else if (blocktype == MPI_INT)
            s = sizeof(int);
          else if (blocktype == MPI_FLOAT)
            s = sizeof(float);
          else if (blocktype == MPI_CHAR)
            s = sizeof(char);
          memcpy((char *)target + block_offset, (char *)source + block_offset,
...
void ADTOOL_AMPI_incrementAdjoint(int adjointCount, MPI_Datatype datatype,
                                  MPI_Comm comm, void *target, void *source,
                                  void *idx) {
  for (unsigned int i = 0; i < adjointCount; ++i)
    ((revreal *)(target))[i] += ((revreal *)(source))[i];
}

void ADTOOL_AMPI_multiplyAdjoint(int adjointCount, MPI_Datatype datatype,
                                 MPI_Comm comm, void *target, void *source,
                                 void *idx) {
  for (unsigned int i = 0; i < adjointCount; ++i)
    ((revreal *)(target))[i] *= ((revreal *)(source))[i];
...
void ADTOOL_AMPI_divideAdjoint(int adjointCount, MPI_Datatype datatype,
                               MPI_Comm comm, void *target, void *source,
                               void *idx) {
  for (unsigned int i = 0; i < adjointCount; ++i)
    ((revreal *)(target))[i] /= ((revreal *)(source))[i];
...
void ADTOOL_AMPI_equalAdjoints(int adjointCount, MPI_Datatype datatype,
                               MPI_Comm comm, void *target, void *source1,
                               void *source2, void *idx) {
  for (unsigned int i = 0; i < adjointCount; ++i)
    ((revreal *)(target))[i] =
        ((revreal *)(source1))[i] == ((revreal *)(source2))[i];
}

void ADTOOL_AMPI_nullifyAdjoint(int adjointCount, MPI_Datatype datatype,
                                MPI_Comm comm, void *target) {
  for (unsigned int i = 0; i < adjointCount; ++i)
    ((revreal *)(target))[i] = 0.0;
}

AMPI_Activity ADTOOL_AMPI_isActiveType(MPI_Datatype datatype) {
  if (datatype == AMPI_ADOUBLE || datatype == AMPI_AFLOAT)
    return AMPI_ACTIVE;
  return AMPI_PASSIVE;
};
...
void ADTOOL_AMPI_cleanupTypes() {
  if (AMPI_ADOUBLE != MPI_DATATYPE_NULL)
    MPI_Type_free(&AMPI_ADOUBLE);
  if (AMPI_AFLOAT != MPI_DATATYPE_NULL)
    MPI_Type_free(&AMPI_AFLOAT);
}
...
MPI_Datatype ADTOOL_AMPI_FW_rawType(MPI_Datatype datatype) {
  ...
  if (datatype == AMPI_ADOUBLE)
    return MPI_DOUBLE;
  else if (datatype == AMPI_AFLOAT)
    return MPI_FLOAT;
  else if (isDerivedType(dt_idx))
    return getDTypeData()->packed_types[dt_idx];
  else
    return datatype;
}
...
int AMPI_Send(void *buf, int count, MPI_Datatype datatype, int src, int tag,
              AMPI_PairedWith pairedWith, MPI_Comm comm) {
  return FW_AMPI_Send(buf, count, datatype, src, tag, pairedWith, comm);
}

int AMPI_Recv(void *buf, int count, MPI_Datatype datatype, int src, int tag,
              AMPI_PairedWith pairedWith, MPI_Comm comm, MPI_Status *status) {
  return FW_AMPI_Recv(buf, count, datatype, src, tag, pairedWith, comm, status);
}

int AMPI_Isend(void *buf, int count, MPI_Datatype datatype, int dest, int tag,
               AMPI_PairedWith pairedWith, MPI_Comm comm,
               AMPI_Request *request) {
  return FW_AMPI_Isend(buf, count, datatype, dest, tag, pairedWith, comm,
                       request);
}

int AMPI_Irecv(void *buf, int count, MPI_Datatype datatype, int src, int tag,
               AMPI_PairedWith pairedWith, MPI_Comm comm,
               AMPI_Request *request) {
  return FW_AMPI_Irecv(buf, count, datatype, src, tag, pairedWith, comm,
                       request);
}

int AMPI_Wait(AMPI_Request *request, MPI_Status *status) {
  return FW_AMPI_Wait(request, status);
}

int AMPI_Barrier(MPI_Comm comm) { return FW_AMPI_Barrier(comm); }

int AMPI_Gather(void *sendbuf, int sendcnt, MPI_Datatype sendtype,
                void *recvbuf, int recvcnt, MPI_Datatype recvtype, int root,
                MPI_Comm comm) {
  return FW_AMPI_Gather(sendbuf, sendcnt, sendtype, recvbuf, recvcnt, recvtype,
                        root, comm);
}

int AMPI_Scatter(void *sendbuf, int sendcnt, MPI_Datatype sendtype,
                 void *recvbuf, int recvcnt, MPI_Datatype recvtype, int root,
                 MPI_Comm comm) {
  return FW_AMPI_Scatter(sendbuf, sendcnt, sendtype, recvbuf, recvcnt, recvtype,
                         root, comm);
}

int AMPI_Allgather(void *sendbuf, int sendcnt, MPI_Datatype sendtype,
                   void *recvbuf, int recvcnt, MPI_Datatype recvtype,
                   MPI_Comm comm) {
  return FW_AMPI_Allgather(sendbuf, sendcnt, sendtype, recvbuf, recvcnt,
                           recvtype, comm);
}

int AMPI_Gatherv(void *sendbuf, int sendcnt, MPI_Datatype sendtype,
                 void *recvbuf, int *recvcnts, int *displs,
                 MPI_Datatype recvtype, int root, MPI_Comm comm) {
  return FW_AMPI_Gatherv(sendbuf, sendcnt, sendtype, recvbuf, recvcnts, displs,
                         recvtype, root, comm);
}

int AMPI_Scatterv(void *sendbuf, int *sendcnts, int *displs,
                  MPI_Datatype sendtype, void *recvbuf, int recvcnt,
                  MPI_Datatype recvtype, int root, MPI_Comm comm) {
  return FW_AMPI_Scatterv(sendbuf, sendcnts, displs, sendtype, recvbuf, recvcnt,
                          recvtype, root, comm);
}

int AMPI_Allgatherv(void *sendbuf, int sendcnt, MPI_Datatype sendtype,
                    void *recvbuf, int *recvcnts, int *displs,
                    MPI_Datatype recvtype, MPI_Comm comm) {
  return FW_AMPI_Allgatherv(sendbuf, sendcnt, sendtype, recvbuf, recvcnts,
                            displs, recvtype, comm);
}

int AMPI_Bcast(void *buf, int count, MPI_Datatype datatype, int root,
               MPI_Comm comm) {
  return FW_AMPI_Bcast(buf, count, datatype, root, comm);
}

int AMPI_Reduce(void *sbuf, void *rbuf, int count, MPI_Datatype datatype,
                MPI_Op op, int root, MPI_Comm comm) {
  return FWB_AMPI_Reduce(sbuf, rbuf, count, datatype, op, root, comm);
}

int AMPI_Allreduce(void *sbuf, void *rbuf, int count, MPI_Datatype datatype,
                   MPI_Op op, MPI_Comm comm) {
  return FW_AMPI_Allreduce(sbuf, rbuf, count, datatype, op, comm);
}
