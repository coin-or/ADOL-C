...
#define ADOLC_EXT_FCT_V2_COMPLETE                                              \
  fos_forward(iArrLength, iArr, nin, nout, (int *)insz, edfct2->x, edfct2->xp, \
              (int *)outsz, edfct2->y, edfct2->yp, edfct2->context)
...
#define ADOLC_EXT_COPY_TAYLORS_BACK(dest, src)
  ...
  if ((ADOLC_CURRENT_TAPE_INFOS.stats[NO_MIN_MAX]) && (ADOLC_CURRENT_TAPE_INFOS.signature == NULL))
    ADOLC_CURRENT_TAPE_INFOS.signature = myalloc1(ADOLC_CURRENT_TAPE_INFOS.stats[NUM_SWITCHES]);
...
#define TAYLOR_BUFFER up_T
  ...
  nonl_dom =
      (struct IndexElement *)malloc(sizeof(struct IndexElement) * indcheck);
  arg_index = (locint *)malloc(sizeof(locint) *
                               (ADOLC_CURRENT_TAPE_INFOS.stats[NUM_MAX_LIVES]));
  opind = 0;
  sod = (struct IndexElement_sod *)malloc(sizeof(struct IndexElement_sod) *
                                          indcheck);
...
#define UPDATE_TAYLORWRITTEN(X)
  ...
  while (operation != end_of_tape) {
    ...
    case gt_a_p:
...
#if !defined(_NTIGHT_)
      {
        ...
        case neq_a_p:
          retval =
              (revreal)(dp_T0[arg] !=
                        ADOLC_CURRENT_TAPE_INFOS.pTapeInfos.paramstore[arg1]);
          opname = "neq_a_p";
          break;
        case eq_a_p:
          retval =
    ...
    case ref_cond_assign: /* cond_assign */
      ...
      arg2 = get_locint_f();
      {
        ...
        fprintf(DIAG_OUT,
                ...
                "in safe mode, please use tight mode\n");
        adolc_exit(-2, "", __func__, __FILE__, __LINE__);
#endif /* ALL_TOGETHER_AGAIN */
      }
      break;
    ...
    case ampi_gatherv: {
      ADOLC_TLM_AMPI_Gatherv(buf, count, datatype, rbuf, NULL, NULL, rtype, src,
                             comm);
      break;
    }
    case ampi_scatterv: {
      ADOLC_TLM_AMPI_Scatterv(rbuf, NULL, NULL, rtype, buf, count, datatype,
                              src, comm);
      break;
    }
    case ampi_allgatherv: {
      ADOLC_TLM_AMPI_Allgatherv(buf, count, datatype, rbuf, NULL, NULL, rtype,
                                comm);
      break;
    }
    case ampi_bcast: {
  ...
  for (int i = 0; i < indcheck; i++) {
    crs[i] =
        (unsigned int *)malloc(sizeof(unsigned int) * (nonl_dom[i][0] + 1));
    ...
    if (temp == NULL) {
      ...
      if (temp->left == NULL) {
        if (temp->entry < tree->entry) {
          temp->left = (struct IndexElement_sod *)malloc(
              sizeof(struct IndexElement_sod));
          temp = temp->left;
          temp->left = NULL;
          temp->entry = tree->entry;
          sod->entry++;
        ...
        if (temp->entry > tree->entry) {
          temp1 = (struct IndexElement_sod *)malloc(
              sizeof(struct IndexElement_sod));
