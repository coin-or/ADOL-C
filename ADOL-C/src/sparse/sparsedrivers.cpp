...
int jac_pat(int id, int size, short tag, int depen,int indep, const double *basepoint, unsigned int **crs, int *options ){
     ...
     if (options[0] !=0 ) {
        if (id==0) printf("This operation is not yet implemented.\n");
        return rc;
     }
     if ( id==0)
        jac_pat(this_tag,depen,indep,basepoint,crs,options);
     else{
        if (( options[0] < 0 ) || (options[0] > 1 ))
           options[0] = 0; /* default */
        if (( options[1] < 0 ) || (options[1] > 1 ))
        ...
        if (options[1] == 1)
           rc = indopro_forward_tight(this_tag, 0, 0, NULL, crs);
        else
           rc = indopro_forward_safe(this_tag, 0, 0, NULL, crs);
     }
     return(rc);
}
...
int sparse_jac(int id,int size, short tag, int depen, int indep, int repeat, const double* basepoint, int *nnz, unsigned int **rind, unsigned int **cind, double **values, int *options)
{
     ...
     if (id==0){
        ...
        /* compute jacobian times matrix product */
        if (options[3] == 1){
           rc = zos_forward(this_tag,depen,indep,1,basepoint,sJinfos.y);
           if (rc < 0)
              return rc;
           rc = fov_reverse(this_tag,depen,indep,sJinfos.seed_rows,sJinfos.Seed,sJinfos.B);
        }
        else
        {
	  ...
	  if (options[2] == -1)
          ...
          if (options[3] == 1){
		   ...
		   fprintf(DIAG_OUT,"ADOL-C error in parallel sparse_jac() computing zos_forward()\n");
             ...
             rc = fov_forward(this_tag, 0,0, tmp, NULL, NULL, NULL, NULL);
          }
     } // end repeat == 0
...
int hess_pat(
     ...
     if (id == 0) {
        ...
        if (option == 1)
           rc = nonl_ind_forward_tight(this_tag, 1, indep, basepoint, crs);
        else
           rc = nonl_ind_forward_safe(this_tag, 1, indep, basepoint, crs);
     } // end of id == 0
     else {
        if (( option < 0 ) || (option > 2 ))
           option = 0;   /* default */
        if (option == 1)
           rc = nonl_ind_forward_tight(this_tag, 0, 0, NULL, crs);
        else
           rc = nonl_ind_forward_safe(this_tag, 0, 0, NULL, crs);
     }
     return rc;
}
...
int sparse_hess(
     ...
     if( id ==0){
        ...
        for (i = 0; i < sHinfos.p; ++i) {
            ...
            ret_val = fos_forward(this_tag, 1, indep, 2, basepoint, v, &y, &yt);
            MINDEC(ret_val, hos_reverse(this_tag, 1, indep, 1, &lag, X));
            for (l = 0; l < indep; ++l)
                sHinfos.Hcomp[l][i] = X[l][1];
        }
        ...
        for (i = 0; i < tmp; ++i) {
            ...
            MINDEC(ret_val, hos_reverse(this_tag, 0, 0, 1, &lag, NULL ) );
        }
     } // end id else
     return ret_val;
#else
