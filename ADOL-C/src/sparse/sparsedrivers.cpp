...
/*      sJinfos.B = myalloc2(sJinfos.seed_rows,sJinfos.seed_clms);
      for(i=0; i < m; i++)
           for(j=0; j < sJinfos.seed_clms; j++)
               sJinfos.B[i][j] = jac_values[i][j];*/
     sJinfos.B = jac_values;
...
int ADTL_get_seed_matrix(int n, int m, unsigned int *nnz, int *p, unsigned int **pattern, double **&seed)
{
#if HAVE_LIBCOLPACK
    int i;
    unsigned int j;
