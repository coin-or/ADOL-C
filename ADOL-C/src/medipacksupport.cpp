...
struct AdolcMeDiAdjointInterface : public medi::AdjointInterface {
  ...
  AdolcMeDiAdjointInterface(double **adjointBase, double *primalBase,
                            int vecSize)
      : adjointBase(adjointBase), primalBase(primalBase), vecSize(vecSize) {}

  int computeElements(int elements) const { return elements * vecSize; }
  ...
  inline void combineAdjoints(void *b, const int elements,
                              const int ranks) const {
    double *buf = (double *)b;
    for (int curRank = 1; curRank < ranks; ++curRank) {
      for (int curPos = 0; curPos < elements; ++curPos) {
        for (int dim = 0; dim < vecSize; ++dim) {
          buf[calcIndex(curPos, dim)] +=
              buf[calcIndex(elements * curRank + curPos, dim)];
        }
      }
    }
  }
...
private:
  inline int calcIndex(int pos, int dim) const { return pos * vecSize + dim; }
};
...
struct AdolcMediStatic {
  ...
  HandleVector &getTapeVector(short tapeId) { return *tapeHandles[tapeId]; }

  void callHandleReverse(short tapeId, locint index,
                         AdolcMeDiAdjointInterface &interface) {
  ...
  void callHandleForward(short tapeId, locint index,
                         AdolcMeDiAdjointInterface &interface) {
    HandleVector &handleVec = getTapeVector(tapeId);
  ...
  void callHandlePrimal(short tapeId, locint index,
                        AdolcMeDiAdjointInterface &interface) {
    HandleVector &handleVec = getTapeVector(tapeId);
...
void mediInitStatic() { adolcMediStatic = new AdolcMediStatic(); }

void mediFinalizeStatic() { delete adolcMediStatic; }
