...
void fail(int error) {
  ...
  case ADOLC_TAPE_TO_OLD:
    fprintf(DIAG_OUT,
            ...
            "older than %d.%d.%d\n",
            failAdditionalInfo1, ADOLC_NEW_TAPE_VERSION,
  ...
  case ADOLC_WRONG_LOCINT_SIZE:
    fprintf(DIAG_OUT,
            "ADOL-C error: Used tape (%d) was written with "
  ...
  case ADOLC_TAPING_TO_MANY_TAYLOR_BUFFERS:
    fprintf(DIAG_OUT, "ADOL-C error: To many taylor buffers!\n"
                      "              Increase "
                      "ADOLC_GLOBAL_TAPE_VARS.maxNumberTaylorBuffers\n");
    break;
  case ADOLC_TAPING_TO_MANY_LOCINTS:
    fprintf(DIAG_OUT,
            "ADOL-C error: Maximal number (%zu) of live active "
            "variables exceeded!\n\n"
            "Possible remedies :\n\n"
            ...
            (int)sizeof(unsigned short), (int)sizeof(int), (int)sizeof(long));
    break;
  case ADOLC_TAPING_STORE_REALLOC_FAILED:
    fprintf(DIAG_OUT,
            ...
            failAdditionalInfo5, failAdditionalInfo3, failAdditionalInfo4);
    break;
  ...
  case ADOLC_REVERSE_NO_TAYLOR_STACK:
    fprintf(DIAG_OUT,
            "ADOL-C error: No taylor stack found for tape "
            "%d! => Check forward sweep!\n",
            failAdditionalInfo1);
    break;
  case ADOLC_REVERSE_COUNTS_MISMATCH:
    fprintf(DIAG_OUT,
            "ADOL-C error: Reverse sweep on tape %d aborted!\n"
            "              Number of dependents(%zu) and/or "
            "independents(%zu)\n"
            "              variables passed to reverse is "
            "inconsistent\n"
            "              with number recorded on tape(%zu/%zu)!\n",
            ADOLC_CURRENT_TAPE_INFOS.tapeID, failAdditionalInfo3,
            failAdditionalInfo4, ADOLC_CURRENT_TAPE_INFOS.stats[NUM_DEPENDENTS],
  ...
  case ADOLC_WRONG_PLATFORM_32:
    fprintf(DIAG_OUT, "ADOL-C error: Trace was created on a 64-bit platform, "
                      "cannot be opened on 32-bit platform!\n");
    break;
  case ADOLC_WRONG_PLATFORM_64:
    fprintf(DIAG_OUT, "ADOL-C error: Trace was created on a 32-bit platform, "
  ...
  case ADOLC_VEC_LOCATIONGAP:
    fprintf(DIAG_OUT, "ADOL-C error: arrays passed to vector operation do not "
...
char *createFileName(short tapeID, int tapeType) {
  ...
  if (ADOLC_GLOBAL_TAPE_VARS.inParallelRegion == 1) {
    ...
    threadNumberString = malloc(sizeof(char) * (threadNumberStringLength + 2));
          ...
          if (strcmp(pos1 + 1, "TAPE_DIR") == 0) {
            ...
            if (err == 0 && S_ISDIR(st.st_mode)) {
              ...
              for (i = 0; i < 4; i++) {
                char *currpos;
                int fnamelen;
                tapeBaseNames[i] = (char *)calloc(namelen[i] - defdirsize +
                                                      pathlen + pathseplen + 1,
                                                  sizeof(char));
  ...
  *degree = ADOLC_CURRENT_TAPE_INFOS.deg_save;
  ...
  if (ADOLC_CURRENT_TAPE_INFOS.lastTayBlockInCore != 1) {
    if (fseek(ADOLC_CURRENT_TAPE_INFOS.tay_file,
              sizeof(revreal) * ADOLC_CURRENT_TAPE_INFOS.nextBufferNumber *
                  ADOLC_CURRENT_TAPE_INFOS.stats[TAY_BUFFER_SIZE],
              SEEK_SET) == -1)
    ...
    for (i = 0; i < chunks; ++i)
      if ((failAdditionalInfo1 =
               fread(ADOLC_CURRENT_TAPE_INFOS.tayBuffer + i * chunkSize,
                     chunkSize * sizeof(revreal), 1,
                     ADOLC_CURRENT_TAPE_INFOS.tay_file)) != 1)
        fail(ADOLC_TAPING_FATAL_IO_ERROR);
    remain = number % chunkSize;
    if (remain != 0)
      if ((failAdditionalInfo1 =
               fread(ADOLC_CURRENT_TAPE_INFOS.tayBuffer + chunks * chunkSize,
                     remain * sizeof(revreal), 1,
                     ADOLC_CURRENT_TAPE_INFOS.tay_file)) != 1)
        fail(ADOLC_TAPING_FATAL_IO_ERROR);
  }
  --ADOLC_CURRENT_TAPE_INFOS.nextBufferNumber;
...
void write_taylor(locint loc, int keep) {
  ...
  while (ADOLC_CURRENT_TAPE_INFOS.currTay + keep >
         ADOLC_CURRENT_TAPE_INFOS.lastTayP1) {
    for (i = ADOLC_CURRENT_TAPE_INFOS.currTay;
         i < ADOLC_CURRENT_TAPE_INFOS.lastTayP1; ++i) {
      *i = (revreal)*T;
    ...
    keep -=
        ADOLC_CURRENT_TAPE_INFOS.lastTayP1 - ADOLC_CURRENT_TAPE_INFOS.currTay;
    put_tay_block(ADOLC_CURRENT_TAPE_INFOS.lastTayP1);
  }

  for (i = ADOLC_CURRENT_TAPE_INFOS.currTay;
       i < ADOLC_CURRENT_TAPE_INFOS.currTay + keep; ++i) {
    *i = (revreal)*T;
...
void write_scaylors(revreal *x, uint size) {
  ...
  while (ADOLC_CURRENT_TAPE_INFOS.currTay + size >
         ADOLC_CURRENT_TAPE_INFOS.lastTayP1) {
    for (i = ADOLC_CURRENT_TAPE_INFOS.currTay;
         i < ADOLC_CURRENT_TAPE_INFOS.lastTayP1; ++i) {
      *i = x[j];
  ...
  for (i = ADOLC_CURRENT_TAPE_INFOS.currTay;
       i < ADOLC_CURRENT_TAPE_INFOS.tayBuffer + size; ++i) {
...
void put_tay_block(revreal *lastTayP1) {
  ...
  if (number != 0) {
    ...
    for (i = 0; i < chunks; ++i)
      if ((failAdditionalInfo1 =
               fwrite(ADOLC_CURRENT_TAPE_INFOS.tayBuffer + i * chunkSize,
                      chunkSize * sizeof(revreal), 1,
                      ADOLC_CURRENT_TAPE_INFOS.tay_file)) != 1)
        fail(ADOLC_TAPING_FATAL_IO_ERROR);
    remain = number % chunkSize;
    if (remain != 0)
      if ((failAdditionalInfo1 =
               fwrite(ADOLC_CURRENT_TAPE_INFOS.tayBuffer + chunks * chunkSize,
                      remain * sizeof(revreal), 1,
                      ADOLC_CURRENT_TAPE_INFOS.tay_file)) != 1)
        fail(ADOLC_TAPING_FATAL_IO_ERROR);
...
void get_taylors(locint loc, int degree) {
  ...
  while (ADOLC_CURRENT_TAPE_INFOS.currTay - degree <
         ADOLC_CURRENT_TAPE_INFOS.tayBuffer) {
    for (i = ADOLC_CURRENT_TAPE_INFOS.currTay - 1;
         i >= ADOLC_CURRENT_TAPE_INFOS.tayBuffer; --i) {
      --T;
      *T = *i;
    }
    degree -=
...
void initTapeBuffers() {
  ...
  if (ADOLC_CURRENT_TAPE_INFOS.opBuffer == NULL)
    ADOLC_CURRENT_TAPE_INFOS.opBuffer = (unsigned char *)malloc(
        ADOLC_CURRENT_TAPE_INFOS.stats[OP_BUFFER_SIZE] * sizeof(unsigned char));
  if (ADOLC_CURRENT_TAPE_INFOS.locBuffer == NULL)
    ADOLC_CURRENT_TAPE_INFOS.locBuffer = (locint *)malloc(
        ADOLC_CURRENT_TAPE_INFOS.stats[LOC_BUFFER_SIZE] * sizeof(locint));
  if (ADOLC_CURRENT_TAPE_INFOS.valBuffer == NULL)
    ADOLC_CURRENT_TAPE_INFOS.valBuffer = (double *)malloc(
        ADOLC_CURRENT_TAPE_INFOS.stats[VAL_BUFFER_SIZE] * sizeof(double));
  if (ADOLC_CURRENT_TAPE_INFOS.opBuffer == NULL ||
  ...
  ADOLC_CURRENT_TAPE_INFOS.lastLocP1 =
...
static void save_params() {
  ...
  if (ADOLC_CURRENT_TAPE_INFOS.currVal +
    ...
    while (ip < np) {
      ...
      put_vals_notWriteBlock(
          ADOLC_CURRENT_TAPE_INFOS.pTapeInfos.paramstore + ip, chunk);
      ip += chunk;
      if (ip < np)
...
void close_tape(int flag) {
  ...
  if (flag != 0 || ADOLC_CURRENT_TAPE_INFOS.loc_file != NULL) {
    ...
    fwrite(&adolc_id, sizeof(ADOLC_ID), 1, ADOLC_CURRENT_TAPE_INFOS.loc_file);
...
static void read_params(TapeInfos *tapeInfos) {
  ...
  valBuffer =
      (double *)malloc(tapeInfos->stats[VAL_BUFFER_SIZE] * sizeof(double));
...
void set_param_vec(short tag, size_t numparam, revreal *paramvec) {
  ...
  if (ADOLC_CURRENT_TAPE_INFOS.stats[NUM_PARAM] != numparam) {
    fprintf(DIAG_OUT,
            "ADOL-C error: Setting parameters on tape %d "
  ...
  if (ADOLC_CURRENT_TAPE_INFOS.pTapeInfos.paramstore == NULL)
    ADOLC_CURRENT_TAPE_INFOS.pTapeInfos.paramstore = (double *)malloc(
        ADOLC_CURRENT_TAPE_INFOS.stats[NUM_PARAM] * sizeof(double));
...
void read_tape_stats(TapeInfos *tapeInfos) {
  ...
  if (tapeInfos->inUse != 0 && tapeInfos->tapingComplete == 0)
    return;
  ...
  tapeVersion = 100 * tape_ADOLC_ID.adolc_ver + 10 * tape_ADOLC_ID.adolc_sub +
                1 * tape_ADOLC_ID.adolc_lvl;
  if (tapeVersion < limitVersion)
    fail(ADOLC_TAPE_TO_OLD);
...
void init_rev_sweep(short tag) {
  ...
  if (ADOLC_CURRENT_TAPE_INFOS.stats[OP_FILE_ACCESS] == 1) {
    ...
    number = (ADOLC_CURRENT_TAPE_INFOS.stats[NUM_OPERATIONS] /
              ADOLC_CURRENT_TAPE_INFOS.stats[OP_BUFFER_SIZE]) *
             ADOLC_CURRENT_TAPE_INFOS.stats[OP_BUFFER_SIZE];
    fseek(ADOLC_CURRENT_TAPE_INFOS.op_file, number * sizeof(unsigned char),
          SEEK_SET);
    number = ADOLC_CURRENT_TAPE_INFOS.stats[NUM_OPERATIONS] %
             ADOLC_CURRENT_TAPE_INFOS.stats[OP_BUFFER_SIZE];
    if (number != 0) {
  ...
  ADOLC_CURRENT_TAPE_INFOS.currOp = ADOLC_CURRENT_TAPE_INFOS.opBuffer + number;
  ...
  if (ADOLC_CURRENT_TAPE_INFOS.stats[LOC_FILE_ACCESS] == 1) {
    ...
    number = (ADOLC_CURRENT_TAPE_INFOS.stats[NUM_LOCATIONS] /
              ADOLC_CURRENT_TAPE_INFOS.stats[LOC_BUFFER_SIZE]) *
             ADOLC_CURRENT_TAPE_INFOS.stats[LOC_BUFFER_SIZE];
    fseek(ADOLC_CURRENT_TAPE_INFOS.loc_file, number * sizeof(locint), SEEK_SET);
    number = ADOLC_CURRENT_TAPE_INFOS.stats[NUM_LOCATIONS] %
             ADOLC_CURRENT_TAPE_INFOS.stats[LOC_BUFFER_SIZE];
    if (number != 0) {
      chunkSize = ADOLC_IO_CHUNK_SIZE / sizeof(locint);
  ...
  if (ADOLC_CURRENT_TAPE_INFOS.stats[VAL_FILE_ACCESS] == 1) {
    ...
    number = (ADOLC_CURRENT_TAPE_INFOS.stats[NUM_VALUES] /
              ADOLC_CURRENT_TAPE_INFOS.stats[VAL_BUFFER_SIZE]) *
             ADOLC_CURRENT_TAPE_INFOS.stats[VAL_BUFFER_SIZE];
    fseek(ADOLC_CURRENT_TAPE_INFOS.val_file, number * sizeof(double), SEEK_SET);
    number = ADOLC_CURRENT_TAPE_INFOS.stats[NUM_VALUES] %
             ADOLC_CURRENT_TAPE_INFOS.stats[VAL_BUFFER_SIZE];
    if (number != 0) {
      chunkSize = ADOLC_IO_CHUNK_SIZE / sizeof(double);
...
void put_op_reserve(unsigned char op, unsigned int reserveExtraLocations) {
  ...
  if (ADOLC_CURRENT_TAPE_INFOS.currLoc + maxLocsPerOp + reserveExtraLocations >
      ADOLC_CURRENT_TAPE_INFOS.lastLocP1) {
    size_t remainder =
  ...
  if (ADOLC_CURRENT_TAPE_INFOS.currVal + 5 >
      ADOLC_CURRENT_TAPE_INFOS.lastValP1) {
    locint valRemainder =
        ADOLC_CURRENT_TAPE_INFOS.lastValP1 - ADOLC_CURRENT_TAPE_INFOS.currVal;
...
void put_op_block(unsigned char *lastOpP1) {
  ...
  for (i = 0; i < chunks; ++i)
    if ((failAdditionalInfo1 =
             fwrite(ADOLC_CURRENT_TAPE_INFOS.opBuffer + i * chunkSize,
                    chunkSize * sizeof(unsigned char), 1,
                    ADOLC_CURRENT_TAPE_INFOS.op_file)) != 1)
      fail(ADOLC_TAPING_FATAL_IO_ERROR);
  remain = number % chunkSize;
  if (remain != 0)
    if ((failAdditionalInfo1 =
             fwrite(ADOLC_CURRENT_TAPE_INFOS.opBuffer + chunks * chunkSize,
                    remain * sizeof(unsigned char), 1,
                    ADOLC_CURRENT_TAPE_INFOS.op_file)) != 1)
      fail(ADOLC_TAPING_FATAL_IO_ERROR);
  ADOLC_CURRENT_TAPE_INFOS.numOps_Tape += number;
...
void get_op_block_r() {
  ...
  fseek(ADOLC_CURRENT_TAPE_INFOS.op_file,
        sizeof(unsigned char) * (ADOLC_CURRENT_TAPE_INFOS.numOps_Tape - number),
        SEEK_SET);
  chunkSize = ADOLC_IO_CHUNK_SIZE / sizeof(unsigned char);
  chunks = number / chunkSize;
  for (i = 0; i < chunks; ++i)
...
void put_loc_block(locint *lastLocP1) {
  ...
  for (i = 0; i < chunks; ++i)
    if ((failAdditionalInfo1 =
             fwrite(ADOLC_CURRENT_TAPE_INFOS.locBuffer + i * chunkSize,
                    chunkSize * sizeof(locint), 1,
                    ADOLC_CURRENT_TAPE_INFOS.loc_file)) != 1)
      fail(ADOLC_TAPING_FATAL_IO_ERROR);
  remain = number % chunkSize;
  if (remain != 0)
    if ((failAdditionalInfo1 =
             fwrite(ADOLC_CURRENT_TAPE_INFOS.locBuffer + chunks * chunkSize,
                    remain * sizeof(locint), 1,
                    ADOLC_CURRENT_TAPE_INFOS.loc_file)) != 1)
      fail(ADOLC_TAPING_FATAL_IO_ERROR);
  ADOLC_CURRENT_TAPE_INFOS.numLocs_Tape += number;
...
void get_loc_block_r() {
  ...
  fseek(ADOLC_CURRENT_TAPE_INFOS.loc_file,
        sizeof(locint) * (ADOLC_CURRENT_TAPE_INFOS.numLocs_Tape - number),
        SEEK_SET);
  chunkSize = ADOLC_IO_CHUNK_SIZE / sizeof(locint);
  chunks = number / chunkSize;
  for (i = 0; i < chunks; ++i)
...
void put_vals_writeBlock(double *vals, locint numVals) {
  ...
  ADOLC_PUT_LOCINT(ADOLC_CURRENT_TAPE_INFOS.lastValP1 -
                   ADOLC_CURRENT_TAPE_INFOS.currVal);
  put_val_block(ADOLC_CURRENT_TAPE_INFOS.lastValP1);
  /* every operation writes 1 opcode */
  if (ADOLC_CURRENT_TAPE_INFOS.currOp + 1 ==
...
void put_val_block(double *lastValP1) {
  ...
  for (i = 0; i < chunks; ++i)
    if ((failAdditionalInfo1 =
             fwrite(ADOLC_CURRENT_TAPE_INFOS.valBuffer + i * chunkSize,
                    chunkSize * sizeof(double), 1,
                    ADOLC_CURRENT_TAPE_INFOS.val_file)) != 1)
      fail(ADOLC_TAPING_FATAL_IO_ERROR);
  remain = number % chunkSize;
  if (remain != 0)
    if ((failAdditionalInfo1 =
             fwrite(ADOLC_CURRENT_TAPE_INFOS.valBuffer + chunks * chunkSize,
                    remain * sizeof(double), 1,
                    ADOLC_CURRENT_TAPE_INFOS.val_file)) != 1)
      fail(ADOLC_TAPING_FATAL_IO_ERROR);
  ADOLC_CURRENT_TAPE_INFOS.numVals_Tape += number;
...
void get_val_block_r() {
  ...
  fseek(ADOLC_CURRENT_TAPE_INFOS.val_file,
        sizeof(double) * (ADOLC_CURRENT_TAPE_INFOS.numVals_Tape - number),
        SEEK_SET);
  chunkSize = ADOLC_IO_CHUNK_SIZE / sizeof(double);
  chunks = number / chunkSize;
  for (i = 0; i < chunks; ++i)
  ...
  ADOLC_CURRENT_TAPE_INFOS.currVal = ADOLC_CURRENT_TAPE_INFOS.lastValP1 - temp;
}
...
locint get_val_space(void) {
  ADOLC_OPENMP_THREAD_NUMBER;
  ADOLC_OPENMP_GET_THREAD_NUMBER;
  if (ADOLC_CURRENT_TAPE_INFOS.lastValP1 - 5 <
      ADOLC_CURRENT_TAPE_INFOS.currVal) {
    ADOLC_PUT_LOCINT(ADOLC_CURRENT_TAPE_INFOS.lastValP1 -
                     ADOLC_CURRENT_TAPE_INFOS.currVal);
    put_val_block(ADOLC_CURRENT_TAPE_INFOS.lastValP1);
  ...
  return (ADOLC_CURRENT_TAPE_INFOS.lastValP1 -
          ADOLC_CURRENT_TAPE_INFOS.currVal);
}
    ...
    if (ip > 0) {
      ...
      if (remain != 0)
        if (fread(ADOLC_CURRENT_TAPE_INFOS.valBuffer + chunks * chunkSize,
          ...
          fail(ADOLC_EVAL_VAL_TAPE_READ_FAILED);
      ADOLC_CURRENT_TAPE_INFOS.numVals_Tape -= number;
      ADOLC_CURRENT_TAPE_INFOS.currVal = ADOLC_CURRENT_TAPE_INFOS.lastValP1;
    }
  }
}
...
int upd_resloc_inc_prod(locint temp, locint newlhs, unsigned char newop) {
  ADOLC_OPENMP_THREAD_NUMBER;
  ADOLC_OPENMP_GET_THREAD_NUMBER;
  if (ADOLC_CURRENT_TAPE_INFOS.currLoc - ADOLC_CURRENT_TAPE_INFOS.locBuffer < 3)
    return 0;
  if (ADOLC_CURRENT_TAPE_INFOS.currOp - ADOLC_CURRENT_TAPE_INFOS.opBuffer < 1)
    return 0;
  if (temp == *(ADOLC_CURRENT_TAPE_INFOS.currLoc - 1) &&
