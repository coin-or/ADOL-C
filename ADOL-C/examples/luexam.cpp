...
void Lsolve(const size_t n, const advector &A, const advector &p, advector &b,
            advector &x) {
  for (size_t j = 0; j < n; j++) {
    x[j] = b[p[j]];
...
void matvec(const double *const A, const size_t m, const double *const b,
            const size_t n, double *const ret) {
  size_t i, j;
...
void residue(const double *const A, const size_t m, const double *const b,
             const size_t n, const double *const x, double *const ret) {
...
double normresidue(const double *const A, const size_t m, const double *const b,
                   const size_t n, const double *const x) {
...
int main() {
  ...
  cout << "COMPUTATION OF LU-Factorization with pivoting (ADOL-C Documented "
          "Example)\n\n";
  ...
  for (size_t k = 0; k < n; k++) {
    cout << "computing gradient of element " << k + 1
         << " of solution w.r.t. matrix elements and rhs\n";
    ansbar[k] = 1.0;
    ...
    for (size_t j = 0; j < n; j++) {
      ...
      cout << "gradient w.r.t. rhs times column " << j + 1
           << " of matrix  = " << scprod << "\n";
    }
    ansbar[k] = 0.0;
  }
