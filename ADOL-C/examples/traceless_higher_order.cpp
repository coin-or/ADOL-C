...
int main() {
  ...
  if (d == 2) // analytical derivative only for d=2 available
  {
    ...
    cout << "ADOLC:    Value of primal: " << y.getValue()
         << "    Value of derivative: " << factorial(d) * ret[0] << endl;
    cout << "DOUBLE:   Value of primal: " << dy << "    Value of derivative: "
         << (exp(dz[0]) *
             (-2 + 4 * dz[0] + dz[0] * dz[0] * log(dz[0] * dz[0]))) /
                (dz[0] * dz[0])
         << endl;
  }
  ...
  cout << "ADOLC:    Value of primal: " << y.getValue()
       << "    Value of derivative: " << factorial(d) * ret[0] << endl;
  cout << "DOUBLE:   Value of primal: " << dy
       << "    Value of derivative: " << higher_order_analytic(dz, d) << endl;
