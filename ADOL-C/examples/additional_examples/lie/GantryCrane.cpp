...
int main() {
  ...
  trace_on(TAPE_F);
  {
    ...
    af[0] = aX[2];
    af[1] = aX[3];
    af[2] = (ml * l * pow(aX[3], 2) * sin(aX[1]) +
             ml * g * sin(aX[1]) * cos(aX[1])) /
            (ml * pow(sin(aX[1]), 2) + mc);
    af[3] = -(ml * l * pow(aX[3], 2) * sin(aX[1]) * cos(aX[1]) +
              (ml + mc) * g * sin(aX[1])) /
            (l * (ml * pow(sin(aX[1]), 2) + mc));
  ...
  for (int i = 0; i <= d; i++) {
    for (int j = 0; j < m_H; j++)
      cout << "dLfh_" << i << "_" << j << " =\t" << gradient[j][0][i] << "\t"
           << gradient[j][1][i] << "\t" << gradient[j][2][i] << "\t"
           << gradient[j][3][i] << endl;
    cout << endl;
  }
