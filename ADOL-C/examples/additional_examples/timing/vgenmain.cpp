...
int main() {
  ...
  if ((pFW < 1) && (controlParameters[cpFovFW] || controlParameters[cpHovFW])) {
    fprintf(stdout, "    # of vectors in vector forward mode = ? ");
    fscanf(stdin, "%d", &pFW);
    fprintf(stdout, "\n");
  }

  if ((pRV < 1) && (controlParameters[cpFovRV] || controlParameters[cpHovRV])) {
  ...
  if (t01 - t00) {
    ...
    fprintf(stdout, "          ");
    fprintf(stdout, TIMEFORMAT, 1.0, (t01 - t00) / evalCount);
  } else {
  ...
  if (controlParameters[cpFosFW]) {
    fprintf(stdout, "--------------------------------------------------------");
    fprintf(stdout,
            "\nTASK %d: forward(tag, m=%d, n=%d, d=1, keep, X[n][d+1], "
            "Y[m][d+1])\n",
            taskCount++, depDim, indepDim);
  ...
  if (controlParameters[cpHosFW]) {
    fprintf(stdout, "--------------------------------------------------------");
    fprintf(stdout,
            "\nTASK %d: forward(tag, m=%d, n=%d, d=%d, keep, X[n][d+1], "
            "Y[m][d+1])\n",
  ...
  if (controlParameters[cpFovFW]) {
    fprintf(stdout, "--------------------------------------------------------");
    fprintf(stdout,
            "\nTASK %d: forward(tag, m=%d, n=%d, p=%d, x[n], X[n][p], y[m], "
            "Y[m][p])\n",
  ...
  if (controlParameters[cpHovFW]) {
    fprintf(stdout, "--------------------------------------------------------");
    fprintf(stdout,
            "\nTASK %d: forward(tag, m=%d, n=%d, d=%d, p=%d, x[n], X[n][p][d], "
            "y[m], Y[m][p][d])\n",
    ...
    for (i = 0; i < evalCount; i++)
      retVal =
          forward(tag, depDim, indepDim, degree, pFW, indeps, XPPP, YP, YPPP);
    t01 = myclock();
  ...
  if (controlParameters[cpHovRV]) {
    fprintf(stdout, "--------------------------------------------------------");
    fprintf(stdout,
            "\nTASK %d: reverse(tag, m=%d, n=%d, d=%d, p=%d, U[p][m], "
            "Z[p][n][d+1], nz[p][n])\n",
  ...
  if (controlParameters[cpTensor]) {
    fprintf(stdout, "--------------------------------------------------------");
    fprintf(stdout,
            "\nTASK %d: tensor_eval(tag, m =%d, n=%d, d=%d, p=%d, X[n], "
            "tensor[m][dim], S[n][p])\n",
  ...
  if (controlParameters[cpInvTensor] && (depDim == indepDim)) {
    fprintf(stdout, "--------------------------------------------------------");
    fprintf(stdout,
            "\nTASK %d: inverse_tensor_eval(tag, m=n=%d, d=%d, p=%d, X[n], "
            "tensor[n][dim], S[n][p])\n",
