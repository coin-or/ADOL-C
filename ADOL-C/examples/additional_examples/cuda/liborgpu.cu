...
__global__ void portfolio_kernel(double *inx, double *outy, double *dery) {
  ...
  // determine which derivative (with respect to which variable) is calculated
  // in a thread
  if (index1 < N)
...
int main() {
  ...
  cudaErr = cudaMemcpy(devx, host, sizeof(double) * npath * (N + Nmat),
  ...
  cudaErr =
      cudaMemcpy(hostres, devy, npath * sizeof(double), cudaMemcpyDeviceToHost);

  // copy values of gradients from GPU to host
  cudaErr = cudaMemcpy(hostder, dery, npath * (N + Nmat) * sizeof(double),
