...
int y_index(int i, int j) { return j + (N_ + 2) * i; }

int u_index(int i, int j) {
  return (N_ + 2) * (N_ + 2) + (j - 1) + (N_) * (i - 1);
}

int pde_index(int i, int j) { return (j - 1) + N_ * (i - 1); }
...
double d_cont(double x1, double x2, double y, double u) { return -exp(y) - u; }
...
template <class T>
bool MyADOLC_sparseNLP::eval_obj(Index n, const T *x, T &obj_value) {
...
template <class T>
bool MyADOLC_sparseNLP::eval_constraints(Index n, const T *x, Index m, T *g) {
  T val;
  ...
  for (int i = 1; i <= N_; i++) {
    for (int j = 1; j <= N_; j++) {
      ...
      val = 4. * x[y_index(i, j)] - x[y_index(i - 1, j)] -
            x[y_index(i + 1, j)] - x[y_index(i, j - 1)] - x[y_index(i, j + 1)];

      // Add the forcing term (including the step size here)
      val += hh_ * d_cont(h_ * 1. * i, h_ * 1. * j, x[y_index(i, j)],
                          x[u_index(i, j)]);
      g[pde_index(i, j)] = val;
    }
  }
...
bool MyADOLC_sparseNLP::eval_f(Index n, const Number *x, bool new_x,
                               Number &obj_value) {
...
bool MyADOLC_sparseNLP::eval_grad_f(Index n, const Number *x, bool new_x,
                                    Number *grad_f) {
...
bool MyADOLC_sparseNLP::eval_g(Index n, const Number *x, bool new_x, Index m,
                               Number *g) {
...
void MyADOLC_sparseNLP::finalize_solution(SolverReturn status, Index n,
...
void MyADOLC_sparseNLP::generate_tapes(Index n, Index m, Index &nnz_jac_g,
                                       Index &nnz_h_lag) {
