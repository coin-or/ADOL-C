...
struct HelmholtzParameters {
  ...
  constexpr HelmholtzParameters(double TE, double R, double coeff, double r,
      ...
      : TE_(TE), R_(R), coeff_(coeff), r_(r), dimIn_(dimIn), dimOut_(dimOut) {};
};
...
T energy(const std::array<T, params.dimIn_> &x,
  ...
  bx = 0;
  he = 0;
  for (size_t i = 0; i < params.dimIn_; ++i) {
    he += x[i] * log(x[i]);
    bx += bv[i] * x[i];
    tem = (2.0 / (1.0 + to_double(i) + to_double(i))) * x[i];
    for (size_t j = 0; j < i; ++j)
      tem += (1.0 / (1.0 + to_double(i) + to_double(j))) * x[j];
    xax += x[i] * tem;
  }
  xax *= 0.5;
