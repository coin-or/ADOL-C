...
/*--------------------------------------------------------------------------*/
/*                                                         The fic function */
int pdficfj(int n, double *x, double *fvec, double r__,
            int nIntern) { /* Initialized data */
  ...
  static double w[5], nf, hm, dw[40] /* was [5][8] */,
      rhnfhk[1280]
      /* was [4][8][8][5] */,
      rhoijh;
  static int eqn, var;
  ...
  for (m = 0; m <= 4; ++m) {
    for (i__ = 1; i__ <= 4; ++i__) {
      ...
      for (j = 0; j <= 7; ++j) {
        nf = 1.;
        for (k = 0; k <= 7; ++k) {
          rhnfhk[i__ + (j + (k + (m << 3) << 3) << 2) - 1] = rhoijh / nf;
          nf *= (double)(k + 1);
        }
        rhoijh *= rho[i__ - 1];
      }
  ...
  for (i__ = 1; i__ <= i__1; ++i__) {
      ...
      for (m = 1; m <= 5; ++m) {
        ...
        for (j = m; j <= 4; ++j) {
          w[m - 1] += rhnfhk[k +
                      ...
                      x[var + j];
          dw[m + j * 5 - 6] =
              rhnfhk[k + (j - m + (j - m + (j - m << 3) << 3) << 2) - 1];
        }
        for (j = 1; j <= 4; ++j) {
          w[m - 1] +=
              rhnfhk[k +
                     (j + 4 - m + (j + 4 - m + (4 - m + 1 << 3) << 3) << 2) -
                     1] *
              x[var + 4 + j];
          dw[m + (j + 4) * 5 - 6] = rhnfhk[k +
                                           (j + 4 - m +
                                                (j + 4
    ...
    for (m = 1; m <= 4; ++m) {
      ...
      for (j = 1; j <= 4; ++j) {
        w[m - 1] +=
            rhnfhk[(j + 4 - m + (4 - m + 1 << 3)) * 32] * x[var + 4 + j];
        dw[m + (j + 4) * 5 - 6] = rhnfhk[(j + 4 - m + (4 - m + 1 << 3)) * 32];
      }
    }
    if (i__ == nIntern) {
      goto L230;
...
int dficfj(int n, adouble *x, adouble *fvec, double r__, int nIntern) {
  ...
  static adouble h__;
  static int i__, j, k, m;
  static adouble nf, hm, dw[40] /* was [5][8] */,
      rhnfhk[1280]
      /* was [4][8][8][5] */,
      rhoijh;
  static int eqn, var;
