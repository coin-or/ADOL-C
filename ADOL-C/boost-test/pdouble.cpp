...
BOOST_AUTO_TEST_CASE(ExpOperator_ZOS_Forward) {
  ...
  zos_forward(*tapePtr, dim_out, dim_in, 0, ad_in.data(), out.data());
  BOOST_TEST(out[0] == ad_in[0] * std::exp(pd_in[0]), tt::tolerance(tol));

  pd_in[0] = 1.2;

  // update pd on tape
  currentTapePtr()->set_param_vec(1, pd_in.data());
...
BOOST_AUTO_TEST_CASE(ExpOperator_FOS_Forward) {
  ...
  fos_forward(*tapePtr, dim_out, dim_in, 0, ad_in.data(), X.data(), out.data(),
              Y.data());
...
BOOST_AUTO_TEST_CASE(ExpOperator_FOS_Reverse) {
  ...
  // Tape setup
  trace_on(*tapePtr, 1); // Reverse mode
  ...
  fos_reverse(*tapePtr, dim_out, dim_in, u.data(), z.data());
  // Check results
  BOOST_TEST(z[0] == aDerivative, tt::tolerance(tol));

  // Update pdouble parameter and recompute
  pd_in[0] = 1.2; // New parameter value
  currentTapePtr()->set_param_vec(1,
                                  pd_in.data()); // Update parameter on tape

  // recompute taylors
  zos_forward(*tapePtr, dim_out, dim_in, 1, ad_in.data(), out.data());
  BOOST_TEST(out[0] == ad_in[0] * std::exp(pd_in[0]), tt::tolerance(tol));

  fos_reverse(*tapePtr, dim_out, dim_in, u.data(),
...
BOOST_AUTO_TEST_CASE(MultOperator_ZOS_Forward) {
  ...
  zos_forward(*tapePtr, dim_out, dim_in, 0, X.data(), Y.data());
...
BOOST_AUTO_TEST_CASE(MultOperator_FOS_Forward) {
  ...
  fos_forward(*tapePtr, dim_in, dim_in, 0, X.data(), Xd.data(), Y.data(),
              Yd.data());
...
BOOST_AUTO_TEST_CASE(FmaxOperator_ZOS_Forward_1) {
  ...
  pd_in[0] = 3.7;
  pd_in[1] = 3.5;
  currentTapePtr()->set_param_vec(2, pd_in.data());
...
BOOST_AUTO_TEST_CASE(FmaxOperator_FOS_Forward_1) {
  ...
  fos_forward(*tapePtr, dim_out, dim_in, 0, X.data(), Xd.data(), Y.data(),
              Yd.data());
  ...
  fos_forward(*tapePtr, dim_out, dim_in, 0, X1.data(), Xd1.data(), Y1.data(),
              Yd1.data());
...
BOOST_AUTO_TEST_CASE(FmaxOperator_ZOS_Forward_2) {
  ...
  std::vector<double> y(dim_out);
  zos_forward(*tapePtr, dim_out, dim_in, 1, ad_in.data(), y.data());
...
BOOST_AUTO_TEST_CASE(FmaxOperator_FOS_Forward_2) {
  ...
  fos_forward(*tapePtr, dim_out, dim_in, 1, ad_in.data(), xd.data(), out.data(),
              yd.data());
...
BOOST_AUTO_TEST_CASE(FminOperator_FOS_Forward_3) {
  ...
  std::vector<double> X{1.0};
  std::vector<double> Y(dim_out);
  fos_forward(*tapePtr, dim_out, dim_in, 0, &a, X.data(), out.data(), Y.data());
  BOOST_TEST(out[0] == expected, tt::tolerance(tol));
  BOOST_TEST(Y[0] == expected_derivative, tt::tolerance(tol));
}
...
BOOST_AUTO_TEST_CASE(FminOperator_FOS_Reverse_3) {
  ...
  std::vector<double> u(dim_out, 1.0);
  std::vector<double> z(dim_in);
  zos_forward(*tapePtr, dim_out, dim_in, 1, &a, out.data());
  fos_reverse(*tapePtr, dim_out, dim_in, u.data(), z.data());
  BOOST_TEST(z[0] == expected_derivative, tt::tolerance(tol));
}
